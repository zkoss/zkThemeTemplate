...
| P3 元件掃描 74 檔 | **進行中** | G-zero + 逐步人工確認 | **步 3 完成:63/74 檔 —— 批 2 收工。**`files differing: 0`;來源端 63 `.css` + 14 `.less` = **77** ✓。位元組層 **37/77 逐 byte 相同**,其餘 40 檔(含 P1 就已知的 `font-awesome`)**全部只差 5 類封閉的序列化寫法**,全樹沒有一個語意 byte 不同。剩 11 檔 = 批 3 / 步 4 | 步 0 `194f8f4`;步 1 `a046bdb`…`370d45b`;步 2 `aa3fac3`…`fbe44bf`;步 3 `5d12946`…`657a890`(工具 `8da83ed`、`82bea4d`、`7f8cd23`、`13da402`) | 2026-08-03 |
...
| 批 3 | 輸出端 >200 條 | **11** | 步 4 | 待確認後開始。**開工前要先拍板 L-8**(`combo` 的 `each()` 攤平,計畫書 §P3〈`combo` 的迴圈〉) |
| | | 20+43+11 = **74** ✓ | | |
...
| 4 | **解巢造成的註解孤兒:9 個「主體只剩註解」的空殼規則**,註解要說明的子規則已被 LESS 提到殼外。處置是**把註解搬到它說明的規則上面、再刪掉空殼**,不是整段刪掉(內容有用:`/* ZK-2151: … */`、`/* Bug 2949287 */`) | `nav`(6)、`borderlayout`(1)、`paging`(1)、`tree`(1) | P3 步 3(`13da402` 修好偵測器後才看得見) |

> **第 1 與第 4 項是同一個輸出現象、兩個不同來源。** 六個 empty-rule 輸出裡,4 檔是步 1 的
> `//` → `/* */` 改寫**造成**的(第 4 項),2 份 `tbeditor` 是來源**本來就**有區塊註解(第 1 項)。
> 兩者都不是真正空的規則 —— **全樹一個真正空的規則都沒有**。
