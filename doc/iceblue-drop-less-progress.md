...
| 批 | 範圍 | 檔數 | 步階 | 狀態 |
|---|---|---|---|---|
| 批 1 | 輸出端 ≤20 條 | **20**(原估 ~8) | 步 0 `tablelayout` → 步 1 `cardlayout`/`absolutelayout`/`anchorlayout`/`grid` → 步 2(+15) | TODO |
...
| `P3` | 元件轉換,可加 `{batch:1\|2\|3}` | ⚠ 需先加 `{step}`,見下 |
...
> **⚠ `P3` 的最小單位跟計畫書對不上(2026-07-31 發現,尚未修)。** 腳本目前最小是
> `{batch:1}` = **20 檔**;計畫書 §2.6 改成步階制之後,步 0 是 **1 檔**(`tablelayout`)。
> 要加 `{step:0|1|2|3|4}`(或 `{limit:N}`),而且**每一步結束要回傳後停住**,不自動續跑。
> 在補上之前不要用 `{batch:1}` 開工 —— 那會直接跳過步 0–2 的三個確認點,
> 也就是跳過步階制的全部作用。
