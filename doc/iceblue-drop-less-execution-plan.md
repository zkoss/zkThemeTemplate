...
| **輸出檔脫離 LESS** | **74 / 77 = 96%** | 核心命題已零差異證明完畢;剩 3 個**刻意**保留的 holdout |
...
| **builder 覆蓋檔數 = 75** | `check:build-css` 真正經過 `build-css.js` 的檔(77 − `norm` − `tablet`,那兩檔是 passthrough、**不是證據**) | — |
...
| **holdout** | 刻意留在 LESS、不歸 P3 管的進入點。**P3 收工時是 3 檔**:`norm`(P5)、`font-awesome`(P6)、`tablet`(P7);**P6 收工後是 2 檔**(`norm`、`tablet`)。**這是會隨階段遞減的數字,不是不變量** —— 引用時要標明是哪個時點 | 把 `3` 當常數;`元件檔 = 74` 那一列用的是 **P3 時點**的 3,那一列不會變 |
...
| **G-delta** | P4a、P4b、P5、P7 | 差異必須**逐條對應到已決策的變更**,且總數符合預估。**每個階段只允許一種 diff 形狀** —— 這是 P4 拆成 P4a / P4b 的理由 |
...
2. **視覺回歸是輔助,不是主閘門。** declaration diff 在「瀏覽器實際收到的 CSS」這一層是
   **完整的等價證明**(DSP 求值不變)⇒ **P0–P3、P6、P8 不需要截圖**,硬加只會製造假的安心感。
   ...
   「誰被選到」)、P7 中等(機制換了)、**P4 最低**(前綴死不死是瀏覽器支援政策問題,截圖答不出來)。
...
| **第 4 層** | **獨立驗證 agent**:唯讀、不給它實作者的複核包、自帶量測、回報格式固定、與計畫書衝突時兩個數字都報、事後用 `git status --porcelain` 驗證它真的沒動 repo | 另一個人/agent | **P3 之後每一步收工** |
...
| | |
|---|---|
| **目標** | 移除 **945** 條 A 群前綴宣告(`border-radius` 532 / `transform` 181 / `box-shadow` 168 / `box-sizing` 60)。逐 rule 實測:945 條**全部**有無前綴同伴,無同伴 0 條 → **零判斷成分** |
| **輸入 → 輸出** | P3 產出的 74 個 `.css` → 同一批檔,少 945 條宣告 |
| **驗收閘門** | **G-delta** —— **只允許 `- <prefixed>`**,且屬性必須在 A 群清單內。出現任何 `+` 記錄就是 bug;上限 **945** |
| **前置** | **L-2**(瀏覽器支援聲明);視覺 A/B harness 已架好並自我驗證過 |
| **commit 粒度** | 一顆 |
...
| | |
|---|---|
| **目標** | **143** 條手寫前綴宣告逐條判斷 —— **全部的判斷都在這 13%**。其中 **26** 條所在的 rule 裡根本沒有標準宣告,必須**換成標準宣告**而不是移除 |
| **輸入 → 輸出** | 同上 → 143 條之內的移除與成對替換 |
| **驗收閘門** | **G-delta** —— 允許 `- <prefixed>`,或**成對**的 `- <prefixed>` + `+ <standard>`;**落單即為 bug**。**B 群 44 條 carve-out 不得出現在 diff 裡** |
