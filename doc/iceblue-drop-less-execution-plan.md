...
→ ~~**P3 已開工:步 0 + 步 1 完成(5/74)**~~ → **步 2 完成,批 1 收工(20/74)。** 它是
G-zero、不需要任何政策決策,而且是工作量主體。做完就已經證明了核心命題(「IceBlue 不需要
LESS」),剩下的都是有意識的取捨。

**現在等你的只有一件事:確認步 2(15 檔,批 1 收工),然後才進步 3(+43 檔 = 批 2)。**
步階制的規則就是每步結束停下等確認(§2.6),所以這裡真的會停住。複核包見進度文件
〈步 2 的複核包〉。三個重點:`NO_HEADER` 三檔**全部走過**(`listbox`/`tree` 補完 `grid`);
**第一次出現位元組不同的轉換**(15 檔中 5 檔),因此複核第 1 層對這些檔結構上不可用、
由第 2 層承擔 —— 全樹複核後確認 6 個差異檔**只差空白與前導零**,零個語意 byte;
另外開了〈P3 來源清理待辦〉共 3 項,全部排在 P3 之後(會改輸出,不屬於 G-zero)。
~~步 0 另外留了一個待拍板:縮排 2 空格還是 tab~~ → **已拍板 tab**
(2026-08-03,`82bea4d`),趁只有 1 檔時改掉了。
