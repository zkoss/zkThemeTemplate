...
→ **P3 已開工:步 0 + 步 1 完成(5/74)。** 它是 G-zero、不需要任何政策決策,而且是工作量
主體。做完就已經證明了核心命題(「IceBlue 不需要 LESS」),剩下的都是有意識的取捨。

**現在等你的只有一件事:確認步 1(4 檔),然後才進步 2(+15 檔)。** 步階制的規則就是每步結束
停下等確認(§2.6),所以這裡真的會停住。5 檔全部與 baseline 逐 byte 相同,複核包見進度文件
〈步 1 的複核包〉。~~步 0 另外留了一個待拍板:縮排 2 空格還是 tab~~ → **已拍板 tab**
