...
| **holdout** | 刻意留在 LESS、不歸 P3 管的進入點。**P3 收工時是 3 檔**:`norm`(P5)、`font-awesome`(P6)、`tablet`(P7);**P6 收工後是 2 檔**(`norm`、`tablet`);**P5 收工後(2026-08-06)是 1 檔**(`tablet`);**P7 第一段收工後(2026-08-17)是 0 檔**。**這是會隨階段遞減的數字,不是不變量** —— 引用時要標明是哪個時點 | 把 `3` 當常數;`元件檔 = 74` 那一列用的是 **P3 時點**的 3,那一列不會變 |
...
> **上表的示例數字停在 P3 時點,定義本身不變。** 現況(2026-08-17):輸出檔數 **85**、
> builder 覆蓋 **85**、holdout **0**(~~2026-08-06:輸出端條數 14863、覆蓋 84、holdout 1~~)。
> 分母是 ZK 10.4 補齊帶動的(+8 檔),分子最後由 P7 第一段補滿。
> 權威值一律看進度文件 L1 / L2,不要引用本表的數字當現況。
