...
每跑一次 `cssdiff` 加一行。**P8 的最終核帳要靠這份紀錄把 ~~P4 + P5 + P7~~
`P4a + P4b + D1/D2 + D4` 的 delta 加總對上,所以不能事後補、不能改寫既有列。**
**←公式於 2026-08-14 更正(C21)**:原式漏掉 D1/D2 的 +352、卻點名了貢獻 0 的 P5;
同日補上本表缺的 D1/D2 那一列(**#59**)—— 補的是**紀錄**,日期照實寫補記日,不回填。
**同一個作法 2026-08-17 再用一次**:D6 完工時漏記,以 **#62** 補上(同日,所以沒有日期落差)。
**兩次都是同一種漏法** —— 閘門跑了、數字寫在 commit 訊息裡,但沒有落到這張表上。
...
| 59 | 2026-08-14 | **D1/D2 —— `data-density` runtime 覆寫層(補記,見 C21)** | `baseline/` **+ 已核准的 P4a + P4b delta** | `target/classes/web/iceblue11` | **1** | **0 移除 / 352 新增** | **PASS(G-delta)** —— **這一列是 2026-08-14 補記的,不是 2026-08-12 回填的**,兩者的差別要講清楚:D1/D2 在 2026-08-12 完工並通過自己的驗收(逐 token 對 `iceblue_c 11.0.0` oracle **350 條值全同、缺 0 條**,證據在 `tasks/l4-density-mechanism.md` L3.5),但**當時沒有在本表留下任何一列** —— 而計畫書 §P8 指名本表為核帳資料來源 ⇒ 少這一列,P8 就核不出這 352 條的來歷。**因此補的是「紀錄」,不是「日期」**:日期欄寫實際補記日 2026-08-14,量測也是當天重跑的,沒有假裝它發生在 8/12。**352 = 350 + 2**,逐項拆得開:`npm run check:gate` **exit 0**,`node scripts/cssdiff.js baseline target/classes/web/iceblue11 --max-diff 5000` 得 **48 檔 / 1104 筆 = 745 移除 + 359 新增**;按檔拆 `+`:`zul/css/norm.css.dsp` **355**、`js/zul/inp/css/slider.css.dsp` **2**、`js/zkex/pdfviewer/css/pdfviewer.css.dsp` **2**,其中 slider 2 + pdfviewer 2 + norm 3 = **P4b 的 7 條改名**,norm 剩下的 352 就是本列:**帶 `data-density` 的宣告 350 條**(`grep -c 'data-density'` 於 `+` 行實測 = 350)**+ 2 條 DSP 指令**(`<c:if test="${'compact' eq c:property('org.zkoss.zul.theme.density')}">` 與 `</c:if>`,在 cssdiff 裡標成 `-- DSP directives: 2 difference(s)`)。**差異檔數 1** —— D1/D2 只動 `norm.css.dsp`,而全樹差異檔數維持 **48**(它本來就在集合內),所以這一列**沒有讓任何檔第一次進入差異集合**。**0 移除**是這一列最重要的形狀斷言:密度層是**純附加**,不刪任何既有宣告 ⇒ 未設 `data-density` 時瀏覽器收到的 CSS 與 baseline 逐條相同。第 1、2 層由 `scripts/density-delta.js` 把這 352 條從 baseline 側抵銷,機制與 `p4a-delta.js` / `p4b-delta.js` 同形 |
| 60 | 2026-08-17 | **P7 第一段 —— `tablet.less` 轉純 CSS(最後一個 entry)** | `baseline/` **+ 已核准的 P4a + P4b + D1 delta** | `target/classes/web/iceblue11` | **0** | **0** | **PASS(G-zero)** —— commit `2f348677`。**681 條、`files differing: 0`**,而且全樹差異檔數維持 **48 檔 / 1104 筆**(對**原始** `baseline/` 的儀器讀數)⇒ 這一段**沒有貢獻任何一筆宣告差異**。三個獨立層全綠:`check:gate` **0**、`check:bytes` **0**(`UNEXPLAINED: 0`)、`check:build-css` **0**。**本列最強的一個讀數是 `check:build-css` 的覆蓋率**:`85 converted (real source), 0 reconstructed from LESS`、`passthrough: 0` —— 在此之前 `tablet.css.dsp` 是唯一**永遠**走不過 CSS 路徑的檔(選擇器位置帶 `<c:if>`,CleanCSS 會在 0 errors / 0 warnings 下改寫它),現在 **85/85 全部由真實 `.css` 來源經 `build-css.js` 產生**。針對本階**新增**的風險(DSP 遮罩)另下專門斷言:14 個 `browserDefault` 開關**逐 byte 還原**、輸出零 placeholder 洩漏、來源零 DSP tag。`zklessc` 本次 `compiled 0 file(s)` —— 樹裡**只剩 `_` 開頭的 partial**,這是 P8 的前置狀態而不是故障 |
| 61 | 2026-08-17 | **P7 第二段 / D4 —— compact 平板層併入 `tablet.css.dsp`(zkmax 側 LESS 歸零)** | `baseline/` **+ 已核准的 P4a + P4b + D1 delta** | `target/classes/web/iceblue11` | **1** | **0 移除 / 618 新增** | **PASS(G-delta)** —— commit `238a3604`。delta 形狀與 D1 同型(1 檔 / 0 移除 / 附加一整段),這次是 **207 個規則區塊 / 618 條宣告**包在 `<c:if>` 裡。**本階的驗收不是靜態閘門而是三態實測**(`npm run check:tablet-density`,讀 ZK 真正服務出去的位元組):`unset` **26207 B / 895 = 895 條對 `baseline/`,缺 0 多 0 值異 0**;`compact` **24308 B / 869 = 869 條對出貨中的 `iceblue_c 11.0.0` jar,缺 0 多 0 值異 0**;`foo` 與 `unset` **逐條相同**(不認得的值不得切換密度)。**§D4 訂的「67 條逐一比對」在這裡被全量取代** ——不是抽驗變全驗,是**單位換掉了**:867/869 那兩個數字涵蓋了兩張表的每一條宣告。**負向控制有跑**:把 `ne` / `eq` 對調重建 → exit 1,而且交叉檢查直接點名「`unset`: served the compact sheet」,那正是 `ne` 帶進來的失效模式,單看位元組大小看不出來。靜態三層照舊全綠:`check:gate` **0**、`check:bytes` **0**(`UNEXPLAINED: 0`)、`check:build-css` **0**(85/85 真實來源、passthrough 0)。D1/D2/D3 未受影響(`check:density-property` **0**、`check:density-runtime` **0**)。`zklessc` `compiled 0 file(s)`,`zkmax/less/` 整棵消失,全樹只剩 `zul/less/` 的 4 個 partial |
| 62 | 2026-08-17 | **D6 —— density 收斂成單一 library-property(C25);桌機側 `[data-density]` 屬性鍵退場**(補記)| `baseline/` **+ 已核准的 P4a + P4b + D1 + D4 delta** | `target/classes/web/iceblue11` | **0** | **0** | **PASS(G-zero)** —— commit `1efde772`。**這一列是同日補記的**:D6 的 commit 訊息裡三支閘門都跑過也都是 0,但**沒有在本表留下一列**,而本表是 §P8 核帳的**唯一**來源(**C21**)⇒ 照 **#59** 的作法補,日期照實寫、不回填。**已核准 delta 的形狀不變** —— `density-delta.js` 仍回報 **1 個規則區塊 / 350 條宣告 / 14473 B**,所以 `P4a + P4b + D1/D2 + D4` 的加總不受影響。**D6 對輸出的實際貢獻已獨立量到,不是引用 commit 訊息**:對**未調整**的 `baseline/` 的全樹原始讀數 **1754 → 1756 筆、49 檔不變**,多出來的 **2 筆全是 DSP 指令**(`<c:if …density…>` 與 `</c:if>`);那 350 條宣告是**同數改鍵**,`+ [data-density="compact"] \|\| …` 350 條原封不動變成 `+ :root \|\| …` 350 條,**宣告淨增 0**。量法:把現行輸出複製一份,只把那一段的選擇器與條件式還原成 D4 當時的樣子再跑 `cssdiff`(D6 對**輸出**只動了這一段,其餘改動全落在 scripts / Java / 文件 / 預覽頁,已逐檔確認),腳本用完即刪。同時重跑:`check:gate` **exit 0**、`check:bytes` **UNEXPLAINED 0**、`check:build-css` **OK(85 converted、passthrough 0,62 檔位元組差異全部落在封閉的序列化類別內)**、`check:density-css` **350 條不變** |
| 63 | 2026-08-17 | **P8 —— LESS 歸零:刪 4 個 partial、移除 `zkless-engine` 依賴與 pom 的 `compile-less` execution、`baseline/` 退回 gitignore、readme + 遷移指南** | `baseline/` **+ 已核准的 P4a + P4b + D1 + D4 delta** | `target/classes/web/iceblue11` | **0** | **0** | **PASS(G-zero)—— 本案的最終核帳,逐段對上、零殘留。** 對**未調整**的 `baseline/` 的全樹原始讀數 **85 檔 / 14941 條 / 49 檔差異 / 1756 筆**,逐段拆帳:**移除 745 = P4a 731 + P4b 14**;**新增 1011 = P4b 7(norm 3 / slider 2 / pdfviewer 2)+ D1/D2 352(350 宣告 + 2 條 DSP)+ D4 652(628 宣告 + 24 條 DSP)**;745 + 1011 = **1756 ✓**,檔數 45 ∪ 9 ∪ {tablet} = **49 ✓**。**D4 這一段的 628 與 `tablet-delta.js` 自報的 618 差 10,原因早已寫在該腳本 `:112`** —— `${".z-page "}` 這種 EL 留下的字面 `{}` 會被當成 10 個幻影規則區塊、帶 10 條幻影宣告,`cssdiff` 不剝 EL 所以看到 628,delta 腳本剝掉所以是 618(該腳本明文記錄「218/628 with the EL left in, 208/618 without」)⇒ **不是多出 10 條真宣告**。**三支複核同時綠燈,而且是在 `zkless-engine` 真的不在 `node_modules` 裡的情況下跑的**(`npm install` 後 `removed 41 packages`,`node_modules/.bin` 只剩 `lessc`):`check:gate` **exit 0**、`check:bytes` **UNEXPLAINED 0**、`check:build-css` **files differing 0 / 85 檔全部來自真來源 / passthrough 0 / byte-identical 23-85、其餘 62 檔全落在封閉序列化類別內**。另外三項端到端:`mvn clean package` **exit 0**,jar 內 **85 個 `.css.dsp` / 0 個原始 `.css` 或 `.less`**;`check:density-property` 與 `check:tablet-density` 兩支三態閘門 **全綠**(它們會真的起預覽程式,所以同時證明 Maven 路徑在沒有 `compile-less` execution 之下仍然可用);`check:doc-refs` **exit 0**。來源樹 `.less` = **0** |
| 64 | 2026-08-18 | **D7 —— density 進入兩張表的 cache key(C28);執行期切換成為規格** | `baseline/` **+ 已核准的 P4a + P4b + D1 + D4 delta** | `target/classes/web/iceblue11` | **0** | **0** | **PASS(G-zero)—— 本階一個 CSS 檔都沒動**,改的是 `ThemeProvider`、兩支閘門與文件,所以對已核准 delta 的讀數必須逐條不變,而它是。**但這一列真正的量測不在 `cssdiff` 上,而在「伺服器組出來的 bundle」上** —— 因為本階唯一的回歸風險是 provider 換人之後**服務出來的內容**變了,而 `cssdiff` 比的是磁碟輸出、看不到那件事。實測讀數:桌面三態 **526988 / 541383 / 526988 B**、compact 區塊 **0 / 1 / 0**;平板三態 **26207 / 24308 / 26207 B**、對 `baseline/` **895 = 895**、對出貨 `iceblue_c` jar **869 = 869**(缺 0 多 0 異 0)。**全部與 D4/D6 收工時逐字元相同**,所以「加 cache key」確實沒有動到任何一個位元組。**新增的是 URL**:桌面 `_zkiju-iceblue11` → `_zkiju-iceblue11-compact`(`Aide.injectURI`),平板 `tablet.css.dsp` → `tablet.css.dsp?density=compact`(query string —— `DspExtendlet` 的 loader 沒有 `_zkiju-` 剝除,注入過的路徑會 404)。**預設密度的 URL 逐字元未變**,既有 app 的快取不被無故失效。**兩支閘門各加一條跨狀態斷言,皆先確認 RED 再實作**(桌面 537634↔526988 同鍵、平板 26207↔24308 同鍵),並各跑負向控制 ⇒ exit 1 且指名 compact 共用鍵。**一個被閘門攔下來的靜默破壞**:provider 第一版寫成 `extends org.zkoss.zul.theme.StandardThemeProvider`,而 ZK 的 provider 是 edition 鏈(`zkmax` → `zkex` → `zul`),只有 zkmax 那節改寫 `~./js/zkex/` 與 `~./js/zkmax/`;繼承最底層再 `setCustomThemeProvider(true)` 讓 `ConfigParser.java:570` 跳過 zkmax 的宣告 ⇒ 服務出來的 bundle **537634 B(+10646)**、`.z-colorbox` 長回 `-moz-`/`-o-`。**`check:bytes` 當時是綠的** —— 這一列因此也是「兩個閘門看的不是同一個東西」的紀錄。改為委派 + 延遲解析後回到 526988。瀏覽器級驗證(出貨快取設定、暖機兩次讓裸 URL 進快取):桌面正常導覽與 reload **兩者都拿到新密度**(修復前兩者都 stale);平板以 iPad UA 實測 `disabled:false`、26207 → 24308、`cached == fresh`。完整推導 `doc/density-runtime-switch-verification.md` |
...
<details>
<summary><b><s>34</s> <s>62</s> 65 條狀態層更正</b> —— 全部只改敘述,<b>量測數字一律不動</b>;既有的閘門紀錄列不改寫(<b>數字自 S34 起就沒跟上,2026-08-17 重數更正</b>)</summary>
...
- **L3-A〈閘門紀錄〉是附加式的** —— 不能事後補、**不能改寫既有列**。
  P8 的最終核帳要靠它把 ~~P4 + P5 + P7~~ `P4a + P4b + D1/D2 + D4`(**C21** 更正)
  的 delta 加總對上,改寫一列就核不了帳。更正一律**另開新列**(紀錄 #29 就是這樣做的)。
  **「附加式」不等於「漏掉就永遠不能補」** —— 2026-08-14 的 **#59** 是這個界線的實例:
  D1/D2 完工當時漏記,補的時候**新開一列、日期寫補記日、量測當天重跑**,
  既有列一個字不動。禁止的是**回填**(假裝它當時就記了)與**改寫**,不是「補記」。
...
| S53 | 2026-08-10 | P4b 唯一有爭議的一條(`-ms-touch-action` ×2)雖已按判準選了純移除,但「cropper 該不該設 `touch-action`」本身仍懸著,未裁示 | **裁示選項 A:維持純移除,P4b 不動;該問題另開議題,交由 cropper 元件負責人判斷。** 理由是它是**元件行為問題**,不是主題轉換問題 —— `-ms-touch-action` 只有 IE10 吃,所以 `.z-cropper-holder` **今天在任何現代瀏覽器上根本沒套用 touch-action**;純移除行為不變,補 `touch-action: none` 則是**在觸控裝置上新增行為**。把後者塞進一顆「移除 LESS 前綴」的 commit,將來 bisect 的人找不到行為變更的出處。**這條隨時可逆且很便宜**:兩個 `cropper.css` 各加一行,本階 table 的 #4/#5 由 `remove` 改成 `rename → touch-action`,閘門數字變成 14 移除 / **9** 新增 —— 但那要走**它自己的 G-delta 核准**,不要回頭改這一階的帳。~~**尚未對 ZK 產品端開單**(對外動作,需另行確認)~~ **←2026-08-14 已開單:[ZK-6147](https://zkoss.atlassian.net/browse/ZK-6147)**(`ZK` / Bug / Affects **11.0.0**)。單上兩條路都寫明,由 cropper 負責人裁;**本階的帳不受影響**(P4b 仍是 14 移除 / 7 新增)。開單時查到兩件本表原本沒記的事:產品端來源 `zkmax/…/cropper/less/cropper.less:106` **在 `zkthemebuilder/template` 另有兩份**(每個從樣板長出來的主題都繼承),且該元素**確實吃得到觸控事件**(`Cropper.ts:491-494` 的 `onZMouseDown` 經 `zk/domtouch.ts:89-91` 映到 `touchstart`/`touchmove`)⇒ 這條不是無害殘骸 | 2026-08-10 使用者裁示;2026-08-14 開單 |
...
| S55 | 2026-08-13 | 兩份文件 L1〈總體進度〉的「**里程碑進度 2 / 5 = 40%**(M1、M2 完成)」,以及執行計畫 L1 的「里程碑**還剩三個**」 | **3 / 5 = 60%**(M1、M2、M3);「還剩兩個(M4 的 P7、M5)」。**M3 早在同兩份文件的里程碑表裡就標了 DONE**(P4a 2026-08-07 / P4b 2026-08-10),只有 L1 的彙總數字沒跟上 ⇒ 同一份文件裡的兩張表互相矛盾了 6 天。連帶把進度文件 L1 的「最後更新」由 2026-08-07 補到 2026-08-13(檔內已有 08-10 / 08-12 / 08-13 的內容)。**這是敘述過期,不是量測改變** —— 沒有任何閘門數字被動到,L3-A 的閘門紀錄一列未改 | 2026-08-13 使用者要求判讀「下一步」時發現;兩處彙總數字與各自檔內的里程碑表對照 |
| S56 | 2026-08-14 | 主線兩份文件描述**現況**的地方仍寫 P4a **728** 條、`-webkit-` **313**、儀器原始差異 **48 檔 / 749 筆** | **P4a 現為 731 / 45 檔、`-webkit-` 314、儀器現值 48 檔 / 1104 筆。** 兩次獨立漂移疊在一起:①**ZK 11 同步(2026-08-13)** —— `baseline/` 取得 upstream `calendar` 的 `.z-cell-range-*` 區塊,其**一次** `.borderRadius()` 展開的 `-moz-`/`-o-`/`-ms-` 三臂正好是 3 條新符合資格的移除 ⇒ A 群母體 945 → 948、上限 788 → 791、P4a 728 → **731**;唯一變動的屬性是 `border-radius` 378 → **381**(其餘 13 項直方圖逐項不變),**檔數維持 45**(calendar 本來就在集合內),`-webkit-` 313 → **314**。②**D1/D2 成為第三個已核准 delta**(2026-08-12)—— 儀器 `check:cssdiff` 不做判斷,所以密度覆寫塊也算進原始差異:現值 **1104 = 745 移除(731 + 14)+ 359 新增(P4b 7 + D1 350 + D2 的 2 條 DSP 條件)**,檔數仍 48(D1/D2 只動 `norm.css.dsp`,它本來就在集合內)。**這是敘述過期,不是量測改變,更不是「改一邊去遷就另一邊」** —— `EXPECTED_REMOVALS` 與 `--expect` **兩處各自獨立地**移動到 731,正是 S46 記的那條絆線正常運作;理由早已寫在 `scripts/p4a-delta.js:64-73` 常數旁邊,`tasks/zk11-jar-baseline-visual-ab.md:267` 亦有記載,**只有主線散文沒跟上**。**修法照 S55 的先例**:只改 L1/L2 描述現況的句子,**L3-A 閘門紀錄 #52–#58 與本表 S41/S43/S44/S46/S48/S50 共 13 處的 728 一律不動** —— 那些是當時實際跑出來的讀數,依附加式紀律不改寫 | 2026-08-14 使用者要求「檢查程式碼現況跟文件」時發現;`npm run check:gate` exit 0(P4a 731 / `-webkit-` 314 = 314)、`node scripts/cssdiff.js baseline target/classes/web/iceblue11 --max-diff 5000` 逐筆計號得 745 `-` / 359 `+`、其中帶 `data-density` 的 `+` 恰為 350 |
| S57 | 2026-08-14 | **P7 = BLOCKED**,原因「L-4 —— compact profile 的替代機制未定」;計畫書附錄 L3-F 的 L-4 那一列寫「**仍未定,不要順勢假設**」 | **L-4 拍板採用 ⇒ P7 轉 TODO。本案自此沒有任何階段被決策擋著,BLOCKED 表是空的。** 裁示:採用 `data-density` 屬性 + 350 條 runtime token 覆寫取代 build 期換 jar,`iceblue_c` 未來不出貨。**這一列是狀態變更,不是新的量測** —— 依據早就存在(D1–D3 於 2026-08-12 完工,對 `iceblue_c 11.0.0` oracle 逐 token **350 全同 / 缺 0**;L3.4 五項子未決 2026-08-13 全數結案),文件也早已寫明「解除條件在事實層面已滿足,只差正式裁示」。**之所以仍需要這一列**:本案的紀律是「狀態變更是決策」,所以事實滿足**不等於**狀態改變,否則 BLOCKED 會在沒有人拍板的情況下自己消失。**連帶更動 5 處**:進度文件 L2.2 BLOCKED 表清空 + P7 移入〈已解除的 BLOCKED〉、L2.1 P7 列轉 TODO、L1 里程碑 M4 註記、L1〈下一步〉第 5 項結案;計畫書 §P7〈前置〉兩項一併解除(L-4 拍板 + mobile UA Playwright 專案已實測存在);附錄 L3-F 的 L-4 列改寫;`tasks/l4-density-mechanism.md` 的 D4 / D5 由 BLOCKED 轉 TODO(前置從「L-4 拍板」改為「P7 完工」)。**未動任何 CSS 來源檔,閘門未跑也不需要跑** | 2026-08-14 使用者裁示;依據見 `tasks/l4-density-mechanism.md` L3.5〈D1–D3 的驗收證據〉與 L3.6 Change Log |
| S58 | 2026-08-17 | 計畫書 §P7 的驗收條寫「A1 的 60 條與 A2 的 1 條都不移除 ⇒ `tablet.css.dsp` 應**逐 byte 不變**」,並在同一句話裡說「與 P3 已做過 74 次的驗收完全同形」。**這兩半互相矛盾,而且前半是做不到的** | **改成 P3 真正的形狀:`cssdiff` 逐條 **0**,位元組差異必須全部落在 `classify()` 那份**封閉**的序列化類別清單內(`UNEXPLAINED: 0`)。** 「逐 byte 不變」不是嚴一點的標準,是**錯的標準** —— 它要求 CleanCSS 與 LESS 的序列化器逐字元一致,而兩者本來就不一致,這一點早就量過並且封閉列舉在 `check-build-css.js` 的 `SERIALIZATION_CLASSES`。實測全樹只有 **23 / 85** 檔逐 byte 相同,**P5 自己轉的 `zul/css/norm.css.dsp` 也不在其中**;tablet 落在其中 3 類(`>` / `+` 組合子兩側空白、`opacity:.3` vs `0.3`、`rgba(255, 255, 255, 0)` vs 無空白),27638 → 27729 B,**零語意差異**。**若照原文字面驗收,P7 第一段會被判失敗,而失敗的是驗收條不是轉檔** —— 更糟的是下一步會去「修」一個沒有壞的東西。**這是敘述更正,不影響任何結論**:第一段仍是 G-zero,`files differing: 0` 照樣成立,A1/A2 仍然一條都沒動(681 條、全樹差異檔數維持 48 / 1104) | 2026-08-17 `npm run check:bytes`(23/85 逐 byte、62 檔可解釋、`UNEXPLAINED: 0`)、`npm run check:build-css`、閘門紀錄 **#60** |
| S59 | 2026-08-17 | `scripts/less2css.js` 第 6 步的閘門把 candidate 跟**原始** `baseline/` 比,要求 `files differing: 0` | **改為與 `check-bytes.js` / `check-build-css.js` 同一套:先把 `baseline/ + P4a + P4b + D1` 疊出來當比對目標。** 這支工具的 74 次使用**全部早於 P4a**(2026-08-07),所以「candidate 應等於 `baseline/`」在它身上一直沒過期過 —— P7 是它第一次在 G-delta 世界裡跑,回報 `files differing: 48`。那 48 是**全樹已核准的 delta**,與被轉的檔一點關係都沒有,但它會 `FAIL`、拒絕 commit,並印出「NOT COMMITTED. To undo: …」⇒ **看起來像轉檔壞了**。這與 S41 當初對第 1、2 層做的判斷是同一件事,只是當時沒有人想到還有第三支工具帶著同樣的假設。**修的是儀器不是斷言**:判準仍是字面的 **0**,而不是「0 加上我們先前同意的那些」 | 2026-08-17 P7 第一次執行即命中;修正後同一次轉檔回報 `files differing: 0  (vs baseline/ + P4a 731− / P4b 14−7+ / D1 350+)` |
| S60 | 2026-08-17 | §D4 與 L3.1(e) 記「只在 default 側的選擇器 **67** 條」,並把它當成 D4 的中和清單與驗收清單長度 | **那個單位量不到真正的漏面。** 依 **(單一選擇器, 屬性)** 對重量:default 側 **895** 條宣告、compact 側 **869** 條、兩側都有 **708**(值相同 633 / 值不同 75)⇒ **只在 default 的有 187 條,散在 113 個單一選擇器**。選擇器字串本身也重量為 **66**(不是 67) | **逗號群組會把漏的藏起來**:`.z-a,.z-b{font-size:15px}`(default)對 `.z-a{font-size:14px}`(compact)算「共用選擇器」,不進那 67 條,但 `.z-b` 的 `font-size` 照樣漏過去 —— 這一類另有 **10 條規則 / 14 條宣告**。**這不只是數字更正,它推翻了 §D4 的作法**:照原訂執行會有 187 條要人工寫回,而計畫只點名其中一部分,漏掉的部分**沒有任何東西看得見**。裁示見 **C23**;新機制的漏面是 **0**,所以這 187 不再是待辦 | 2026-08-17 對 `zkmax/css/tablet.css` 與 compact profile 編譯輸出的逐對比對,閘門紀錄 **#61** |
| S61 | 2026-08-17 | §D4 / L3.1(e) 記兩側「規則區塊 260 / 217、宣告 681 / 628(純 CSS)」 | **純 CSS 的真值是 246 / 207 與 667 / 618。** 差額正好是該節自己記的 `browserDefault` DSP 區塊數(default **14**、compact **10**)| L3.1(e) 已經更正過一次(274/695/227/638 → 260/681/217/628,理由是剝掉 DSP 區塊),但**只剝了一半**:`<c:if …>${".z-page "}</c:if>` 裡的 `${…}` 還留著,而它自帶一組 `{}`,於是每個開關又被算成一個規則區塊。同一個坑在 `tablet-delta.js` 的 `measure()` 又踩了一次(218/628 → 208/618),那裡已寫進註解。**不動任何結論**,也不動 **67 / 27** 以外的敘述 —— 真正推翻 §D4 的是 **S60** 不是本列 | 2026-08-17 `node scripts/tablet-delta.js`(**207** 區塊 / **618** 宣告)|
| S62 | 2026-08-17 | **S36 的結案**(S36 曾寫「已拍板:接受到 P7 為止,不另外寫建置期檢查」,並記載只設 `@themeProfile` 會得到「桌機 default + 平板 compact」的分裂主題、而且**沒有任何檢查看得到**)| **S36 結案 —— 分裂主題在結構上不可能再發生,而且不是靠檢查擋住的,是靠那兩個旋鈕都不存在了。** 桌機側的旋鈕(`norm.css` 第一個 `@import`)與平板側的旋鈕(`_zkvariables.less` 的 `@themeProfile`)雙雙退場,取而代之的是**同一個 library-property `org.zkoss.zul.theme.density`,被兩張表各自的伺服器端條件讀一次**(D4/**C23** + D6/**C25**)。**S36 是被兌現而不是被繞過** —— 它當初裁示不寫檢查的理由正是「P7 會把兩個旋鈕一起換掉」,而那件事發生了;**當初「沒有檢查看得到」的缺口,今天有兩支閘門看得到**(`check:density-property` / `check:tablet-density`,皆為三態且各有負向控制)。**S36 那一列不改寫**(附加式紀律),以本列結案。**殘留一項,已知且不影響結論**:`zul/less/_zkvariables.less` 的 `@themeProfile: "default"` 字面仍在,但**已無任何 CSS 讀它**(`zkmax/less/` 整個目錄在 D4 刪除),唯一的讀者是 `gen-var-table.js` 拿它挑範例列,而那支腳本本來就因 **S14 / S37** 是紅的 ⇒ 一併留給 **P8** 的 `.less` 歸零。**對外交付物**:[migration/density.md](migration/density.md) —— C24 / C25 的「只有 property」規格在那裡是正面寫的一節,不是注意事項 | 2026-08-17 `npm run check:density-property`(desktop 三態:`unset` / `foo` **無** compact 區塊、`compact` **有一個且鍵在 `:root`**;bundle 526988 / 541383 / 526988 B)+ `npm run check:tablet-density`(tablet 三態:`unset` / `foo` **26207 B**、895 = 895 對 `baseline/`;`compact` **24308 B**、869 = 869 對 `iceblue_c 11.0.0` jar,缺 0 多 0 異 0),兩支皆 exit 0;`grep -rn themeProfile` 全樹 = 1 條宣告 + 1 處腳本引用 |
| S63 | 2026-08-17 | **L1〈現況〉與 L1〈總體進度〉寫「這不等於『LESS 已經移除』—— 樹裡還有 4 個 partial 與 `zkless-engine` 依賴,那是 P8」**;L2.1 的 P8 那一列狀態 **TODO**;M5 **TODO**;里程碑進度 **4 / 5** | **全部翻面:P8 收工,`.less` = 0、`zkless-engine` 不在依賴裡、pom 沒有 `compile-less` execution ⇒「LESS 已經移除」第一次成立。** M5 DONE、里程碑 **5 / 5**、閘門紀錄 **#63** | **這是狀態更新而不是更正**,列在這裡的理由是那句警告在文件裡出現過四次、語氣很強,單純刪掉會讓讀舊 commit 的人以為它從來沒說過 ⇒ 照附加式紀律**劃掉並註明它到哪一天為止成立**。**新增兩項「未做、已記錄」**(不是漏掉,是不在計畫書的交手清單裡):① `build-css.js` 的 `--watch` —— 引擎的 watcher `ignoreNonLessFiles` **只看 `.less`**,所以 `npm run zklessc-dev` 從 P3 起就一個檔一個檔失去監看對象、**P7 之後完全空轉**,這是 P3–P7 造成、P8 才揭出的既有回歸;`readme.md` 已改為實話。② **S31** 版號一致性腳本。兩項都寫進進度文件〈P8 留下的兩項待裁示〉 | 2026-08-17 `find src -name '*.less'` = **0**;`npm install` → `removed 41 packages`、`node_modules` 內 `zkless` 目錄 **0** 個、`.bin` 只剩 `lessc`;`check:gate` / `check:bytes` / `check:build-css` / `check:density-property` / `check:tablet-density` / `check:doc-refs` **全 exit 0**;`mvn clean package` exit 0,jar 內 **85 `.css.dsp` / 0 原始來源**;`git ls-files baseline/` = **0** 而 `check:baseline` 仍 **86/86 OK** |
| S64 | 2026-08-18 | **S31 只記到「四處只需彼此一致,沒有機制偵測漂移」** —— 沒有記**漂移會怎樣**,也沒有記四處是不是同一種東西。連帶讓那條待辦讀起來像四份對等的字串,「補不補腳本」因此看不出輕重 | **四處不對等,實測分成三類。** ZK 的檢查在 `ConfigParser.checkVersion`(`zk/src/main/java/org/zkoss/zk/ui/sys/ConfigParser.java:132-139`):兩支 XML 各自宣告 `<version-class>` + `<version-uid>`,ZK 載入該類別讀 `UID` 欄位,字串不相等就**整份檔案跳過**,而且只用 `log.info`。⇒ **`Version.java` 是被比對的那一側(單一參照),兩支 XML 各自與它比;`pom.xml` 根本不參與**(它只決定 jar 檔名、`bin` zip 的 `packname`、OSGi `Bundle-Version`)。實測漂移後果:**① `config.xml` 漂 ⇒ 主題整個不生效**(頁面仍 **HTTP 200**、無例外,`_zkiju-iceblue11` 從樣式表 URL **消失**,`iceblue11` 命中 3 → **0**),唯一痕跡是一行 **INFO**;**② `lang-addon.xml` 漂 ⇒ 今天無害**,頁面正規化後與對照組**逐 byte 相同** —— 但無害是**巧合**:本主題的 `lang-addon.xml` 除了自己的版本塊什麼都沒有(無 `<component>`、無 `<javascript>`,與 C9 的發現一致),fork 一旦往裡面加元件,同一個漂移就會靜默吞掉它;**③ `Version.java` 漂 ⇒ 等於兩支 XML 同時漂**(實測兩行 `version not matched`,主題同樣不生效);**④ `pom.xml` 漂 ⇒ 執行期零影響**,壞的是包裝標籤(jar 檔名與主題自報的版本不一致)。**⇒ 真正會殺掉主題的只有 ① 與 ③,而它們的告警等級是 INFO。** 另外量到**沒有任何機制在維持這四份副本**:pom 的 `<resources>` 沒有 `<filtering>`,所以兩支 XML 裡的字串是手打的字面值,不是從 `${project.version}` 代入的 | 2026-08-18 實測:四處今天皆為 `11.0.0-Eval`(S31 當時是 `10.4.0-jakarta-Eval`,**升版有做到四處同步,但那只是一個樣本**)。四次啟動只污染 `target/classes` 的副本、`src/` 一次未動、還原後與 `src/` **逐檔相同**、`git status` 乾淨;對照組 `iceblue11` 命中 **3** / 樣式表 `/_zkiju-iceblue11/zul/css/zk.wcs` |
| S65 | 2026-08-18 | **S64 與當日報告把四處版號稱為「四份副本」**,並據此把「開 Maven resource filtering、讓兩支 XML 讀 `${project.version}`」列為**建議選項**(4 處變 2 處) | **使用者更正:四個版號不是副本,各有不同作用,但應該保持一致。** ⇒ filtering 那個方向**不採用** —— 它消掉的是「不同作用的獨立宣告」,不是「重複的值」。裁示改為**兩支腳本**:`set-version.js` 一次改四處、`check-version.js` 驗四處一致並進 `check:gate`(8 步),四處位置只寫一份(`version-locations.js`)供兩支共用。**S64 量到的東西一條都沒有被推翻**(四處不對等、兩種致命漂移、告警只有 INFO),被推翻的是從那些量測導出的**處置建議**。另外更正 S64 的一句話:「沒有任何機制在維持這四份副本」**不完全對** —— `init.sh` 本來就把它們當四處處理(帶 `___VERSION___` 的**恰好**是這四個檔),只是**只在初始化時維持一次**;缺的一直是初始化之後那一段 | 2026-08-18 使用者裁示;實作證據見進度文件〈S31 裁示:兩支腳本,一支改、一支檢查〉——11 個案例全在拋棄式副本上實測(含 exit 2 的「量不到不算過」與「部分寫入」防護),gate 接線以污染真實 `config.xml` 實測(exit 1、後 5 步未執行、還原後 `git diff` 無差異) |
