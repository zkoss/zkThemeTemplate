...
| `visual:selftest`(桌機) | **150 頁比對、differing 0**(頁數 149 → 150 是 §7.11 新增的場景)。✅ **`avatar` 已定性並登記**:三態,失敗簽章 `75px / maxΔ 10 / box 105,284,135,319` 在兩次獨立失敗中**完全一致**,已進 `KNOWN_UNSTABLE`(附反向控制)。**沒有收緊等待,理由見 §7.13** |
...
### 7.13 `avatar` 的定性:三態、為什麼不收緊等待、以及一個故意不登記的缺口

**先講方法,因為結論全靠它**:單一趟 selftest 是**擲硬幣**,看一次結果什麼都不能斷定
(§5.3 早就寫過)。所以照同樣的方法論做:**用真正的 capture 路徑、開 N 個獨立的瀏覽器
process,數出相異 PNG 的個數**,再把每一對拿去 `png-compare` 比。

**量到的:三個狀態,不是兩個。**

| 狀態 | sha256(前 8) | bytes | 出現頻率 |
|---|---|---|---|
| 常見 | `ac43daa8` | 35370 | 30 個獨立 process 中 **26**;4 趟完整 selftest 的 8 張全部 |
| 近似孿生 | `8676afe3` | 35391 | 30 個中 **4** |
| 罕見 | `9443d1fc` | 35409 | 兩次 selftest 失敗各一次 |

**兩兩比較才是重點**:

| 配對 | 結果 |
|---|---|
| 常見 ↔ 近似孿生 | `12px maxΔ 3` → **noise,低於門檻,根本不會被報出來** |
| 常見 ↔ 罕見 | **`75px maxΔ 10 box 105,284,135,319`** → differs ← 這就是那次失敗 |
| 近似孿生 ↔ 罕見 | `87px maxΔ 10 box 105,284,140,319` → differs,**另一組簽章** |

**是什麼在動**:那個 30×35 的框是 avatargroup 裡**唯一一個帶圖的 avatar 的圓形裁切邊**
(`img/item2.jpg`,原生 98×122,被縮進 40×40 的圓,而 "+5" 那顆從右邊疊上來)。
常見狀態下圖片填滿整個圓;罕見狀態下沿著弧線會露出後面那顆的**灰色月牙**。
`maxΔ 10` 是**弧線的抗鋸齒**,不是元素位移 —— 放大 5 倍目視確認過。

**訂正**:這一頁的不穩定原本被歸因成 `ab-capture.spec.ts` 註解裡那個 **label fallback** race
(`avatar.zul` 故意指一張不存在的圖去展示文字 fallback)。**那是錯的** —— 那些 fallback 的
avatar 在 x=5 / 37 / 69(文字 `HW`/`JL`/`MK`,`hasImg: false`),而差異框在 **x=105..140**,
是那顆**真的有載到圖**的(`item2.jpg`,`imgComplete: true`,`naturalSize 98x122`)。
會誤判是因為那句註解剛好就在 `images complete` 這個等待條件旁邊,讀起來像現成的解釋 ——
**先量元素座標,再挑解釋**。

**為什麼不收緊等待(這是關鍵的否決理由)**:頁面**已經**等了 `images complete`、
`fonts.ready`,還要求**連續兩格畫面相同**(相隔 250ms)。而每一個 process **內部是穩定的**
—— 這正是 `shootStable` 會收斂、然後跟下一個 process 對不上的原因。
**狀態是 per-process 的常數,沒有「更晚的一格」可以等。** 等久一點不會改變它。
在沒有可重現的失敗可以驗證之前就改等待條件,等於交出一個無法驗證的修改。

**為什麼可以登記**:登記的門檻跟 breadcrumb 一樣是「**同一組簽章出現兩次**」。
`75px / maxΔ 10 / box 105,284,135,319` 在**兩次獨立的 selftest 失敗中一模一樣**,
所以 exact-match 條目是有意義的,不是猜的。附反向控制:把記錄的 `75` 改成 `76`,
它立刻又被報成真差異(`rc 1, differing 1, unstable 0`),改回去又match —— 證明這是
**收窄**,不是把整頁的差異一律吞掉。

**故意不登記的缺口**:上表第三列那組 `87px / maxΔ 10 / box 105,284,140,319`
是「近似孿生 ↔ 罕見」**算出來**的,**從來沒有真的在 selftest 裡出現過**。
為一個沒觀察到的差異先寫一條 exact-match,正是這個 registry 的設計要拒絕的猜測。
哪天真的報出這組數字,再加第二條 —— 而 §7.12 那個保留機制會留下 PNG 讓它可診斷。

**順手補上的保留機制(這一節之所以能寫出來的原因)**:第一次失敗的證據**被我自己下一趟
跑掉了** —— selftest 固定寫 `selftest-a`/`selftest-b` 兩個 label,而 `capture()` 開頭就
`rm -rf` 輸出目錄。於是那唯一一張有用的 PNG 沒了,後面 30 個 process + 4 趟完整 selftest
都沒能重現它。現在 `selftest` 失敗時會把差異頁的 a/b 兩張**複製到
`target/ab-visual/selftest-evidence/<timestamp>/`** 並印出路徑。
**它上線後第一次真失敗就派上用場了** —— 這一節的三態分析就是靠那次保留下來的 PNG 做的。
