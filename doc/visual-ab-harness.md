...
> 同一個 build 截兩次 → **pages differing: 0**(連續兩輪),噪音 4–5 頁、全部 ≤42px 且 maxΔ 1。
> **反向控制**:對輸出注入一行 `.z-button{border-radius:12px}` → **36 頁差異**,
> 最小的一頁 370px ⇒ 對噪音上限(64px)有 **≈6 倍**餘裕。詳見 §5。
...
| **不證明** | 變得對不對(那是人看);也不是覆蓋率 —— 頁面用的是 Marble 的語料,**不是**針對 85 個輸出檔設計的 |
...
| **動畫 GIF**(CSS 關不掉) | 主題自帶 5 個動畫 GIF,全在 `zul/img/misc/`;用 `page.route(...).abort()` 攔掉。見 §5 |
...
反向控制證實這個下限有牙齒:注入一行 button 圓角 → 36 頁差異、最小 370px、maxΔ 255。
