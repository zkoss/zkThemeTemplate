# IceBlue 棄用 LESS —— 執行計畫 技術附錄(L3)

> **本檔不定義任何規範。** 階段定義、驗收閘門判準、範圍邊界、**術語表**一律看
> [iceblue-drop-less-execution-plan.md](iceblue-drop-less-execution-plan.md) 的 **L2**;
> 現在做到哪看 [iceblue-drop-less-progress.md](iceblue-drop-less-progress.md)。
>
> 這裡只有**原文保留**的微觀細節:行號、commit hash、byte 差異、實測 log、變更歷史。
> L1/L2 只寫當前結論;**要知道某個結論是怎麼來的、以及它推翻了什麼,看這裡。**
>
> **索引與〈何時要看〉在計畫書的
> [L3 技術附錄](iceblue-drop-less-execution-plan.md#l3-技術附錄獨立檔)一節**,本檔不另立一份 ——
> 一份索引兩個地方維護,遲早對不上。
>
> **摺疊規則**:實作期間會反覆查的兩節(**L3-C** 方法學、**L3-D** 各階細節)**不摺疊**,
> 好讓 `§P4`、`§P8` 這類指涉在頁面上直接找得到;其餘是參考料,收在 `<details>` 裡,
> `<summary>` 帶關鍵數字。
>
> **舊章節編號**(`§0`、`§2.6`、`§P4` 這類)的完整對照在
> [計畫書〈舊章節編號對照〉](iceblue-drop-less-execution-plan.md#舊章節編號對照l3-內文與外部文件仍在用)。

---

### L3-A 決策依據:為什麼要放棄 LESS

<details>
<summary><b>8 條贊成(A1–A8)· 8 條反對(B1–B8)· 淨判斷</b> —— 決定性的是 A1(834/846 個 LESS 變數是純 1:1 pass-through)與 A2(兩套 theming API 互相靜默覆蓋)。最誠實的反對是 B1+B2:拿不到「零 build step」</summary>
...
| **B4** | **23 套付費佈景是產品排程,不是工程排程** | L-7 拍板了**方向**(palette → runtime `--zk-*` sheet),沒拍板**誰做、何時做、綁 ZK 11 哪個里程碑**。Theme Pack 是付費商品,有客戶合約 | ~~❌ **未解決。本案最可能被外力延遲的一點**~~ **←2026-08-13 解除(C19)**:用**切出去**而不是排進來 —— palette 移出本案,獨立計畫 `tasks/theme-pack-palette-mechanism.md` + **M-2**。本案為此要留下的東西是**零**(palette 全是 `:root{--zk-*}` 覆寫,本主題已出貨 862 個可覆寫 token)⇒ **產品排程不再能延遲本案** |
...
<details>
<summary><b>22 條實測前提</b> + 1.1 最關鍵的第 5 點(未壓縮輸出已經是可用的 .css 原始碼)+ 1.2 註解保留的坑 + 1.3 一個 .css.dsp 是怎麼被產出來的(mermaid 流程圖與各節點的程式碼位置)</summary>
...
| # | 事實 | 數據 |
|---|---|---|
| 1 | master 的 LESS 樹能完整編譯 | 77 個 `.css.dsp`,**14323** 條 declaration 記錄(另有 436 條 DSP 指令單獨比對) |
...
| 10 | 規模分佈(**輸出端**) | **2** 檔 ≤4 條;最大 4 檔:font-awesome **4545**、norm **1500**、tablet **681**、combo **586**。**這一列全部是輸出端** —— 來源端數字小得多(例:font-awesome 910),引用時要標明是哪一端 |
...
| 14 | `_zkmixins.less` 是 **30 個名稱 / 38 個定義列** | 數名稱**必須用含連字號的 pattern**:`\w`-only 會**無聲丟掉 6 個帶連字號但仍可呼叫**的 mixin(`.encodeURL-verGradient`、`.gradient-ver/-hor/-diagm/-diagp/-rad`)。名稱 30 與定義列 38 的差額是 LESS 的**同名多載**(依參數個數或 `when` guard 分派)。**兩個數字要成對引用** —— 各取一個湊成的組合描述不了任何檔案 |
...
前三層由「最不需要信任」排到「最需要信任」;第 4 層是**架在第 3 層上的守衛**,補的是「前三層都是
同一個人做的」這個弱點。**P2 只需要第 1 層;P3 要第 1–3 層;P4 起每一步都要四層。**
...
**這一層與第 2 層已經合併成一支腳本**(2026-08-04,`9230348`):

```bash
npm run check:bytes    # 全樹逐 byte 比對 → 差異用下面 5 個封閉類別歸因 → 剩下的 exit 1
```

它與閘門**互相獨立**:`cssdiff` 解析宣告並套 6 條正規化,所以它最強只能說「宣告集合相同」;
`check:bytes` 問的是嚴格更強的位元組問題。它也涵蓋 `check:build-css` 自己標記為 not evidence 的
兩檔(`norm`/`tablet` 在那支檢查裡是原樣複製,所以那支不能對它們發言)。
現況:**77 檔比對、26 檔逐 byte 相同、51 檔差異已歸因、0 檔無法解釋**。
...
##### 第 4 層:獨立驗證 agent(**P3 之後每一步必做**,2026-08-04 拍板)

前三層有一個共同弱點:**都是同一個人做的**。第 1、2 層還撐得住 —— 它們是確定性腳本
(`npm run check:bytes`、`npm run check:build-css`),換誰跑都得到同一組數字,獨立性來自**工具本身**,
再派一個 agent 去跑不會多出任何資訊。**第 3 層不是** —— 複核包是自評,而它偏偏是 G-zero 把
「行為有沒有變」這個問題關掉之後,**唯一還會出錯的地方**。

所以從 P4 起,**每一步收工都要派一個獨立的驗證 agent**:

| 規則 | 為什麼 |
|---|---|
| **唯讀** —— 不改檔、不 build、不跑閘門、不 commit | 驗證者一動手就變成參與者,而參與者不能當自己的證人 |
| **不給它實作者的複核包** | 給了就只會複誦。要的是獨立重新導出,不是同意 |
| **自帶量測**,不引用計畫書或進度文件裡的數字 | P0–P2 的教訓:兩個 agent 都回報「閘門 0 差異 PASS」,其中一個的覆蓋率數字是錯的(13642 vs 12142)—— **閘門結果正確不代表關於閘門的敘述正確** |
| **回報格式固定**,而且明講「空結果是合法且良好的結果」 | 不留「湊幾條交差」的空間 |
| 與計畫書衝突時**兩個數字都報**,不改斷言去符合量測 | 與 §2.1 的硬禁令同一條:分歧本身就是發現 |
| 事後用 `git status --porcelain` 驗證它真的沒動 repo | 禁令要被驗證,不是被相信 —— 這一層的全部精神 |

**已跑過的第一次:P3 收工的 74 檔複審 fan-out(2026-08-04)。** 8 個唯讀 agent、每片約 1675 行,
重複檔對(`goldenlayout`/`cropper`/`signature`)刻意放同一片,免得同一個發現被報兩次。
結果見進度文件〈P3 收工複審〉。

##### 兩個序列化器的 5 個寫法差異(封閉清單)

**全樹經 CSS 路徑的位元組相同率已量(2026-07-31,`npm run check:build-css`)**:
...
| 2 | 產物是 **2 空格縮排**(LESS 預設),repo 的 `.less` 用 **tab** | **已拍板 tab**(2026-08-03,`82bea4d`),趁只有 1 檔時改掉。`build-css.js` 會把縮排壓掉,所以輸出不受影響 —— 這是「74 個新的、要人維護的來源檔」的可讀性決定。當時改是一行,73 檔之後改就是全樹空白 commit |
...
2. **留著 pin 的理由是前瞻性的,不是驗證。** 153 個 `.less` 會在 P3–P7 之間縮到 3 個,而本分支
   ...
   **exit 0** 下改壞 —— 沒有任何 build error 攔得住的失效模式。**三個理由裡只有這一個支持
   「pin 要留下來」。**
3. **成本一行,而且 P8 不會把這個 pin 刪掉。** P8 之後
...
- 批 3(**11** 檔,>200 條):`popup` 217、`menu` 218、`tabbox` 237、`colorbox` 246、
  ...
  —— **`combo` 是唯一 loop-generated 的 P3 檔**,見下方〈`combo` 的迴圈〉
...
`combo.less:3-4` 對 6 個元件名跑 `each()`:
`z-combobox, z-bandbox, z-datebox, z-timebox, z-spinner, z-doublespinner`。

P3 的規則是「拿編譯輸出當新的原始碼」(§1.1),所以**迴圈會被靜默攤平**。
實測 `baseline/js/zul/inp/css/combo.css.dsp`:6 個 per-component 區塊**連續排列**、
大小幾乎一致(3842 / 3806×5 bytes),共用宣告被**物理複製 6 份** ——
`height:var(--zk-combo-input-height)` 出現 **18** 次(6 元件 × 3 處)、
`font-size:var(--zk-input-text-size)` **12** 次、`display:inline-block` **13** 次。

所以 P3 之後 `js/zul/inp/css/combo.css` 會是一個**要人手維護**的 586 條檔案,
共用樣式寫 6 遍,加一條宣告要改 6 個地方。
**這正是 §P6 為 FA 提出的反對理由**(「4545 條、沒有人維護得動」)——
差別是 **combo 有原生解法而 FA 沒有**:combo 的 `each()` 只是把名稱扇開到選擇器前綴,
原生**選擇器清單**就表達得出來。一個必須寫下來的限制:**CSS 原生嵌套的 `&` 不能字串串接**,
所以 `&-input` 不能照搬,必須展開成 `.z-combobox-input, .z-bandbox-input, …` 明列清單 ——
宣告只寫一次,選擇器變長。順帶一提,`combo.less` 的**尾段本來就用選擇器清單**
(`.z-timebox, .z-spinner, .z-doublespinner { … }`),來源自己混用兩種寫法。

**但併起來會破 G-zero**:586 條記錄會塌到 ~100 條,而且來源順序會變
(現在是「combobox 全部規則」→「bandbox 全部規則」的 per-component 分組)。
→ 去重是 **G-delta** 變更,本計畫沒有為它編列階段。
**L-8 已拍板(2026-08-03):先 A 後 B。** 步 4 按 A 案逐字轉完(`d74bf65`),
上面推估的三個數字在轉出來的來源檔上**實測完全命中**(18 / 12 / 13);
去重登記為〈P3 來源清理待辦〉第 5 項。詳見 §6 的 L-8 決定。

**G-zero**:每一檔、每一批,以及全樹。

**每一步還要多過一道人工複核**(§2.6):第 1 層位元組相同 → 第 2 層讀不同的那幾個 byte →
第 3 層逐檔複核包 → **第 4 層獨立驗證 agent**(P3 是收工時補跑一次 74 檔;P4 起每一步都要)。
**閘門過了不等於這一步可以結束** —— 要等確認才進下一步。
...
1. **全樹經 CSS 路徑的位元組相同率:24/75**,而且差異已分類完(§2.6)。
2. **`npm run check:build-css` 已跑,綠燈**(75 檔 / `files differing: 0`
...
3. **workflow 腳本已能表達步階制**(`8da83ed`),見〈執行機制〉。
...
**←已拍板(2026-08-07):選項 C —— 不對 Safari 做任何下限聲明,`-webkit-` 全數保留,
只移除 `-moz-` / `-ms-` / `-o-` / `-khtml-`。** 見
[L3-F〈已拍板:L-2〉](#已拍板l-2--瀏覽器支援聲明2026-08-07-開同日拍板)。
...
| | 允許的 diff | 上限 |
|---|---|---|
| **P4a** | 只有 `- <prefixed>`,且屬性必須在 A 群清單內 | ~~**945**(全數移除的上限)~~ → **788** |
| **P4b** | `- <prefixed>`,或**成對**的 `- <prefixed>` + `+ <standard>` | ~~143 條之內,實際數量由 L-2 決定~~ → **15** |
| 兩階皆然 | B 群 44 條 carve-out **不得出現在 diff 裡**;成對替換落單即為 bug | — |
| **L-2 拍板後新增** | **`-webkit-` 285 條真前綴一律不得出現在 diff 裡**(與 carve-out 同級) | — |

> **上限已由 L-2 收斂(2026-08-07,選項 C:全數保留 `-webkit-`)。**
> 表中劃掉的是拍板前「不分前綴」的口徑 —— 它在當時是對的,但**閘門必須用新數字**,
> 否則 P4a 會放行 157 條不該動的 `-webkit-`(A 群裡的 `-webkit-` 共 237 條)。
> 完整分解與依據見 [L3-F〈已拍板:L-2〉](#已拍板l-2--瀏覽器支援聲明2026-08-07-開同日拍板)。

理由與分群見下一節 —— **下一節的 945 / 143 是「不分前綴」的分群依據,仍然成立**,
L-2 只是在它之上再加一道「`-webkit-` 不動」的過濾。
...
(`_icons.less:7,11`、`_sizing.less:15` 的 `each()`;`_shims.less:314`、`_sizing.less:5`
的 `when` guard 遞迴)。
...
> **界定:全樹的迴圈有 3 個階段(前提 #22),但只有這一階需要「產生器」。**
> P3 的 `combo` 與 P7 的 `tablet/compact/_combo` 也用 `each()`,但那兩處只是把名稱扇開到
> 選擇器前綴 —— **原生選擇器清單就表達得出來**(見 §P3〈`combo` 的迴圈〉、L-8)。
> FA 沒有這個出路:它是 ~700 組 name→codepoint 對應,**清單本身就是資料**。
> 所以上面那段「產生器而不是轉換」的論證**只對 FA 成立**,不要外推 ——
> **不要日後把「迴圈」整體誤記成「產生器問題」。**

**相依於 P2**:產生出來的 `.css` 需要 `build-css.js` 才會變成 `.css.dsp`。P2 未完成就跑 P6,
閘門會把該檔報成 missing —— 那看起來像產生器寫錯,其實不是。順序:P2 → P6。

**G-zero**:**4545** 條 declaration 零差異(來源端 `.less` 是 910 條,`each()`
...
> `tasks/theme-pack-palette-mechanism.md`(**刻意不進本 repo 版控**,見 M-2);
...
**這一階還有兩個 `each()` 迴圈**(前提 #22,原本漏了):
`zkmax/less/tablet/compact/_combo.less:27`(`@comps1`,5 個元件)與 `:36`(`@comps2`,2 個元件)。
**與 §P3 的 `combo` 完全同形狀** —— 只是把名稱扇開到選擇器前綴,原生選擇器清單表達得出來,
同樣受 `&` 不能字串串接的限制(`&-input:not(&-input-full)` 必須展開成明列清單)。
**歸在同一個拍板項 L-8 底下處理**,否則會在 P7 把 P3 已經爭論過的事重新爭論一次。
...
**G-zero**:全樹最終輸出 vs P0 baseline,差異必須完全等於 ~~P4 + P5 + P7 三階段~~
**`P4a + P4b + D1/D2 + D4` 四段**已核准的 delta 總和 —— 不多不少
(**←2026-08-14 更正,C21**)。核帳資料來自進度文件的〈閘門紀錄〉。
...
<details>
<summary><b>13 條風險與對策</b> —— 含「編得過不等於輸出等價」、minifier 取代 LESS 成為新風險源、基準被污染、閘門空轉</summary>


| 風險 | 對策 |
|---|---|
| 「編得過」不等於「輸出等價」 | 每階段 declaration diff;這正是 Phase 0 抓到 `grid-column: 1 / -1` 的方式 |
| minifier 取代 LESS 成為新風險源 | CleanCSS 會摧毀 `@scope`(輸出全空)與裸 `@layer a,b;`(**連後面第一條規則一起清空**)。**光檢查 `output.warnings`(不只 `errors`)不夠 —— P2 實測推翻了這個對策**:選擇器位置的 DSP tag 被改寫成 `${}".z-page "` 時 **0 errors 且 0 warnings**,warnings 檢查完全看不到。實際對策是 `build-css.js` 的 4 條 `HOSTILE_CONSTRUCTS` 前置守衛 —— 命中就**讓 build 失敗**,不是警告。P5/P7 遇到時**不可以放寬守衛**,要「先 minify 內層再包外層」或「先換佔位符再還原」 |
...
| **只有工具說「對」,人看不出對在哪** | `files differing: 0` 是自製工具的判決。對策:§2.6 的四層複核 —— 第 1 層 `npm run check:bytes` 完全不經過 `cssdiff`,一行給答案。P3 每一步都要過前三層,P4 起加第 4 層獨立驗證 agent |
...
<details>
<summary><b>L-2 / L-4 / L-5 / L-7 / L-8</b> 與六次追加拍板(2026-07-30 ×2、07-31 ×2、08-03、08-04),含各自的依據與當時的未決事項</summary>


**P0 已完成並過閘**(`f34ca01`,`files differing: 0`)。

**P2 已完成(`dc46cd3`)。P3 完成:步 4 收工,74/74。** 它是
G-zero、不需要任何政策決策,而且是工作量主體。做完就已經證明了核心命題(「IceBlue 不需要
LESS」),剩下的都是有意識的取捨。

**核心命題已經證明完畢:IceBlue 的 74 個元件輸出不需要 LESS,而且是零差異證明的。**
來源端現在是 **74 `.css` + 3 `.less` = 77** ✓,剩下的 3 個 `.less` 是三個**刻意**留下的
holdout:`norm`(P5)、`font-awesome`(P6)、`tablet`(P7)。閘門 `files differing: 0`
(77 檔 / 14323 條),`build-css: compiled 74 file(s)`。複核包見進度文件〈步 4 的複核包〉。

步 4 的 11 檔逐檔閘門**全 0、無一次失敗**;全樹獨立複核 **26/77 逐 byte 相同**、51 檔不同但
**全部落在 5 類封閉的序列化寫法**、**0 檔無法解釋** ⇒ 零個語意 byte。四個重點:
**複核第 1 層在這一批完全不可用**(11 檔逐 byte 相同 **0** 檔,四步曲線 5/0 → 10/5 → 12/31 → 0/11
—— 是機制必然,不是品質下降);**「同名重複來源必是複本」被反證** —— `goldenlayout` 是複本,
但兩份 `tbeditor` 是**真的兩個上游版本**(Trumbowyg v2.7.2 vs v2.31),已拆成兩個不同的 P8 問題;
**步 3 修的空殼偵測器立刻回本**(又抓到 6 處,全樹 15 處 / 8 檔,真正空的 0 個);
**`combo` 的預估重複量測完全命中**(18 / 12 / 13,3.0× 展開,P3 最大)。
〈P3 來源清理待辦〉現為 **5 項**,全部排在 P3 之後(會改輸出,不屬於 G-zero)。

> **下一步不再是 P3。** 未決的是各後續階段自己的前置:P4 等 L-2 與視覺 A/B harness、
> P6 可以直接開(G-zero,`gen-fa-css.js`)、P7 等 L-4 的 density 那一半、P5 等 A/B harness。

**產物縮排已拍板:tab**(2026-08-03,`82bea4d`),趁只有 1 檔時改掉了。

**P1 已完成(決定 2026-07-30,實作與閘門 2026-07-31)**,
見 §P1 的〈S0 + S1 實作紀錄〉與下方〈追加拍板〉。

**L-2(P4 前置)已拍板(2026-08-07)**,見下。
**L-5(P6 前置)已定**,見下表。
**在 P7 之前要定**:L-4 —— compact profile 的替代機制(**colour 那一半已解除,density 那一半仍未定**,見下表)。
**L-8(P3 步 4 前置)已拍板(2026-08-03)**,見下。

#### 已拍板:L-2 —— 瀏覽器支援聲明(2026-08-07 開,同日拍板)

> **決定:選項 C(保守)—— 全數保留 `-webkit-`,只移除 `-moz-` / `-ms-` / `-o-` / `-khtml-`。**
> ⇒ **本主題不對 Safari 做任何版本下限聲明。**

**為什麼不是「訂一個下限然後全清」。** 利弊分析見
[`tasks/l2-browser-support-analysis.md`](../tasks/l2-browser-support-analysis.md)。三個決定性的點:

1. **IceBlue 是既有預設主題,不是新主題。** ZK 10 → 11 升級的客戶會原地繼承它,
   對它提高瀏覽器下限 = **對存量客戶的 breaking change**。Marble 是全新主題,沒有這個包袱 ——
   兩者沒有理由共用同一句聲明。
2. **拿不到客戶端的實際瀏覽器分布。** C 是唯一**不需要那份資料**就能安全執行的選項。
3. **C 仍然拿到 74% 的清理量**(803 / 1088),而風險是零 —— 移除的 `-moz-` / `-ms-` / `-o-` /
   `-khtml-` 在**任何**下限之下都已經死透(Presto 2013 停更、IE EOL、Firefox 69/80 起無前綴、
   Konqueror 2005 年前)。它們不承載任何「我們支援到哪」的判斷。

**已知並接受的代價**:`-webkit-` 的 285 條真前綴永久留在純 CSS 來源裡。
LESS 時代它們由 mixin 自動展開、維護成本為 0;純 CSS 之後**每次新增一個圓角都要手抄**。
拍板時明確不引入 autoprefixer —— 那等於把剛拆掉的編譯層再裝一個回去。

**這個決定不封死將來。** 若日後拿到瀏覽器分布、或 ZK 11 本體發布了自己的支援聲明,
「再清 285 條 `-webkit-`」是一個獨立、可機械化、可 revert 的 G-delta,不需要重做 P4。

**拍板後量出的分解**(用 `cssdiff.js` 自己的 parser,口徑與閘門的 14863 條相同;
量測腳本一次性,結果如下,詳見利弊分析 §10):

| | 條數 | P4 的處置 |
|---|---|---|
| A 群 non-`-webkit-` | **708** | **P4a** 純移除,diff 只允許 `- <prefixed>` |
| C 群 non-`-webkit-`,有無前綴同伴 | **80** | **P4a** 同上(併入,形狀相同) |
| C 群 non-`-webkit-`,**無**同伴 | **15** | **P4b** 逐條判斷,允許成對 `-`/`+` |
| `-webkit-` 真前綴 | 285 | **保留**,不得出現在 diff |
| B 群 carve-out | 44 | **保留**,不得出現在 diff |
| **合計** | **1132** | 可移除 **803**(P4a 788 + P4b 15) |

> **P4a 的上限從 945 改為 788、P4b 從 143 改為 15。** 舊數字是「不分前綴」的口徑,
> 在 L-2 拍板前是對的;**閘門檢查必須用新數字**,否則 P4a 會放行 157 條不該動的 `-webkit-`。

#### 已拍板:L-8 —— 攤平迴圈的維護成本(2026-08-03 開,同日拍板)

> **決定:先 A 後 B —— A 現在做,B 排成 P3 之後的既定工作,不是「有空再說」。**
> 步 4 已按 A 案把 `combo` 逐字轉完並過閘(`d74bf65`,586 條,`files differing: 0`),
> P3 因此在同一天收工 74/74,G-zero 與步階制都沒有被破壞;
> 去重(B)登記為〈P3 來源清理待辦〉**第 5 項**,與另外 4 項同一個 G-delta 清理階段處理。
>
> **為什麼這個順序不是拖延。** 本節原本自己就寫下了理由:「步 4 一旦把 `combo` 轉完並 commit,
> 選 B 就變成『改一個剛剛才產生的檔』,而不是『決定它怎麼產生』。」——
> 也就是**先轉不會關掉 B 這個選項**,只會把它從「阻擋 P3 收工的政策決策」降級成
> 「一個有明確輸入與輸出的編輯工作」。反過來先做 B 才要付代價:P3 會卡在 73/74,
> 而 B 需要獨立階段 + 逐條對應 + 視覺 A/B,等於讓**唯一一個 G-delta 判斷**擋住
> **73 個已經證明完畢的 G-zero 檔案**收工。
>
> **P7 的兩處一併適用**(`tablet/compact/_combo.less:27,36`):同形狀、同處置 ——
> P7 轉的時候照樣先攤平,去重併入同一項。**不要在 P7 重新爭論一次。**

**範圍**:`js/zul/inp/less/combo.less:4`(P3 批 3,586 條)與
`zkmax/less/tablet/compact/_combo.less:27,36`(P7)。**兩者同形狀,必須一起拍**,
否則 P7 會重新爭論一次。FA(P6)**不在範圍內** —— 它沒有原生解法,走產生器已由 L-5 定案。

**問題**:P3 的「拿編譯輸出當新的原始碼」會把 `each()` 靜默攤平成 **6× 重複宣告**
(實測數字見 §P3〈`combo` 的迴圈〉)。原生 CSS 表達得出來(選擇器清單),
但併起來**不是 declaration-identical**,會破 G-zero。

| 選項 | 代價 |
|---|---|
| **A —— 接受 6× 重複** | G-zero 不動、P3 照跑、不新增階段。代價是留一個 586 條、共用樣式寫 6 遍的**手維護來源檔**給自己與客戶的 fork;每加一條宣告要改 6 個地方 |
| **B —— 併成選擇器清單(G-delta)** | 宣告只寫一次。代價是 586 → ~100 條、來源順序改變(per-component 分組消失)→ 需**獨立階段 + 逐條對應 + 視覺 A/B**;且 `&` 不能字串串接,`&-input` 必須展開成明列清單 |

**期限的由來**:`combo` 落在批 3 / 步 4 —— **期限到了、當天拍板、當天收工**
(見本節開頭的決定)。但步 4 一旦把 `combo` 轉完並 commit,
選 B 就變成「改一個剛剛才產生的檔」,而不是「決定它怎麼產生」 ——
**這句話從「日後的觀察」變成了這次拍板的主要依據。**
...
| **L-4 —— compact profile** | ~~**仍未定,不要順勢假設。**~~ **←2026-08-14 已拍板:採用。** `@themeProfile` 是**密度**軸,不是**顏色**軸,L-7 沒有涵蓋它,所以它單獨拍板。**裁示內容**:採用 `data-density` 屬性 + 350 條 runtime token 覆寫(計畫書 `tasks/l4-density-mechanism.md`),取代 build 期換 jar;**`iceblue_c` 未來不出貨**(L3.4 第 3 項,2026-08-13)。**依據不是提案而是實測** —— D1–D3 已完工並對 `iceblue_c 11.0.0` oracle 逐 token 比對 **350 條值全同、缺 0 條**,視覺 A/B 反向控制 0 頁差異。**這是對外 breaking change**(「改 LESS 變數重編 jar」→「設一個 library-property」),由 **D5** 寫進 migration guide。⇒ **P7 解除 BLOCKED**,L-4 五項子未決已於 2026-08-13 全數結案 |
...
| **產物縮排** | **已拍板 tab**(2026-08-03,`82bea4d`;見 §2.6〈步 0 的結果〉第 2 項)。2 空格 = LESS 預設,tab = repo 現況;`build-css.js` 會壓掉縮排所以輸出不受影響 |

**P8 的接手規格與三個地雷已寫進 §P8**〈拿掉 `zkless-engine`:`build-css.js` 要接手什麼〉——
放在實作者會看的地方,不要只留在這張決策表裡。

#### 追加拍板(2026-08-04,P3 收工)

| 議題 | 決定 |
|---|---|
| **驗證的獨立性要分清楚哪一層真的獨立** | 第 1、2 層獨立性來自**工具**(確定性腳本,換誰跑同一組數字),派 agent 去跑不會多出資訊;**第 3 層是自評**,而 G-zero 關掉「行為有沒有變」之後它是唯一還會出錯的地方。所以新增 §2.6〈第 4 層〉:**P3 之後每一步收工都要派獨立驗證 agent**,唯讀、不看實作者的複核包、自帶量測、事後用 `git status --porcelain` 驗證它沒動 repo |
| **第 1+2 層從一次性腳本變成 repo 腳本** | `npm run check:bytes`(`9230348`)。理由:前四步的第 2 層都是手寫一次性腳本跑的 —— **只有這個 session 能重跑的檢查,對之後的維護者等於不存在**。順帶補了兩個缺口:路徑對 repo root 解析(可從任何 cwd 跑)、**baseline 沒有對應檔的多餘輸出**現在會被報出來(以 baseline 為主的走訪看不見它,而那正是殘留 `.less` 的形狀) |
| **P3 的 74 檔複審 fan-out 要補跑** | 執行機制腳本裡設計好的 `P3: review` 從沒跑過 —— 74 檔的可讀性複審原本只有實作者抽樣看過。已於 2026-08-04 補跑:8 個唯讀 agent、每片約 1675 行 |
...
<details>
<summary><b><s>19</s> 25 條規範層更正</b> —— 前提數字 9 條 · 判準與方法學 10 條(<b>←這個拆帳停在 C19,之後 6 條未歸類</b>)。舊敘述原文保留,不刪除(<b>總數 2026-08-17 重數更正</b>)</summary>

**這一節存在的理由,就是讓 L1/L2 可以只寫當前結論。**
規則是:**假設被推翻就直接改 L1/L2 的結論,變更過程記在這裡** ——
不在正文裡「打補丁」,因為補丁會讓同一個事實在不同段落有不同版本。

配套的兩條硬規則(它們與上面那條相容,不衝突):

- **不改斷言去符合量測**(L3-C)。舊估計**保留**在紀錄裡,不因為量出了新數字就抹掉 ——
  分歧本身就是發現。
- **進度文件的〈閘門紀錄〉是附加式的,既有列一律不改寫**。更正另開新列。

狀態層(複核包用詞、進度敘述)的更正在**進度文件 L3-I**。

#### 前提與數字

| # | 日期 | 被推翻的敘述 | 更正後的結論 | 怎麼發現的 |
|---|---|---|---|---|
| C1 | 2026-07-29 | 前提 #1 declaration 總數 **14807** | **14323**(另有 436 條 DSP 指令單獨比對) | 原數字是**來源端**,閘門比的是**輸出端**。P0 用 `cssdiff` 實測 |
| C2 | 2026-07-29 | 前提 #10 `font-awesome` **910** 條 | **4545** 條(全樹最大檔,不是 `norm`) | 同 C1。連帶改掉 P6 的 G-zero 數字 |
| C3 | 2026-07-29 | 前提 #10 `norm` 1243 / `tablet` 421 / `combo` 407 | **1500 / 681 / 586** | 同 C1 |
| C4 | 2026-07-29 | 前提 #10「**12** 檔 ≤4 條」 | 輸出端只有 **2** 檔 | 同 C1。**直接讓 P3 批 1 的原規劃失效**,改成按輸出端條數分界 |
| C5 | 2026-07-30 | `_zkmixins.less` 是 **24** 個 mixin 名稱 | **30** 個名稱 / **38** 個定義列(必須成對引用) | grep 用 `^\.[a-zA-Z][a-zA-Z0-9]*`,**遇到連字號就截斷** → 無聲少算 6 個可呼叫的 mixin。產生器自帶斷言 + 審核 agent 用另一支獨立 parser 重數才抓到 |
| C6 | 2026-07-30 | 前提 #13 `@iphone`/`@android`「`tablet.less` 使用」 | **全樹 0 引用,兩條都是死的** | `tablet.less` 只是 `@import` 了那個檔 —— 把「import 了檔案」誤當成「用了變數」。P7 不需要為它們編列移植 |
| C7 | 2026-07-30 | P8 規則表例外 **10** 條(後改 12) | **16** 條 —— 12 條分類例外 + 4 條「語法 1:1 但行為不是」 | 窮盡掃過全樹的編譯期函式呼叫站點 |
| C8 | 2026-07-30 | 前綴宣告總數 **1127** 條 | **1132** 條 | 原本只數 `-webkit-`/`-moz-`/`-ms-`/`-o-` 四種,**漏掉 5 條 `-khtml-user-select`** |
| C9 | 2026-07-30 | P3 批次檔數估計 **~8 / ~55 / ~11** | **20 / 43 / 11** | `cssdiff --list` 實測。三個估計值**全部不對**;新數字是斷言,對不上就要停下來查 |

#### 判準與方法學

| # | 日期 | 被推翻的敘述 | 更正後的結論 | 怎麼發現的 |
|---|---|---|---|---|
| C10 | 2026-07-30 | P4 是**一階**,G-delta 形狀是「總數 ≤1127 的純移除」 | **拆成 P4a / P4b,而且允許三種 diff 形狀**:純移除、成對替換、不得出現(carve-out) | 逐 rule 實測發現 26 條前綴**沒有無前綴同伴**,正確處理必然產生 `+` 記錄 → 原判準會把正確的做法判成 bug。同時發現 44 條 carve-out 長得跟可移除的一模一樣 |
| C11 | 2026-07-30 | 「minifier 換成 CleanCSS」 | **必須是 `level 0`** | level 1(= Marble 的設定)實測 **60 檔 / 2034 條差異**;`level:{1:{all:false}}` 關不掉顏色改寫與 selector 排序 |
| C12 | 2026-07-31 | 對策是「檢查 `output.warnings`(不只 `errors`)」 | **不夠** —— 改成 `build-css.js` 的 4 條 `HOSTILE_CONSTRUCTS` 前置守衛,命中就**讓 build 失敗** | 選擇器位置的 DSP tag 被 CleanCSS 改寫成 `${}"…"` 時 **0 errors 且 0 warnings**,warnings 檢查完全看不到 |
| C13 | 2026-07-31 | 5 類序列化寫法清單原本只有 **4** 類 | **5** 類(補上「空規則」) | `check:build-css` 首跑時 `tbeditor` 兩份解釋不了 → 查出來才補。**清單是封閉的才有用** |
| C14 | 2026-07-31 | (曾預期)位元組相同率會像 LESS→LESS 那樣是 **76/77** | 走 CSS 路徑實測 **24/75** | 計畫書當時特地寫了「不要先假設它是 76/77」,量出來確實不是。**P3 的第 1 層鏡片因此改成「相同 or 落在 5 類內」** |
| C15 | 2026-07-31 | P1 是**可選的**,「建議做但不做也站得住」 | **不再是可選的** | L-7 拍板 Theme Pack 走「runtime `--zk-*` sheet + 新 CSS 語法」,而 LESS 3 靜默改壞的四種語法**正好落在那個語法區** → pin 從過渡期護欄升級成**下一步的前提** |
...
| C19 | 2026-08-13 | **`@themePalette` 是 P7 的一半**(L-7 拍板的 runtime `--zk-*` override sheet),連帶 **S29** 是 P7 的交付項、P7 驗收多一條「override sheet 要表達得出 palette 覆蓋 + 一次非 iceblue 實測」 | **全部移出本案。** §P7 = `tablet` + `@themeProfile`;S29 改判**不補**(隨 P8 的 `.less` 歸零消失);palette 的機制、Java API、出貨物獨立成 **M-2** 與 `tasks/theme-pack-palette-mechanism.md` | **user 裁示(2026-08-13),依據是產品邊界而不是工程量**:palette 就是付費商品 Theme Pack 的內容。**連帶解除 L3-A 的 B4**(見該列的 ← 註)。**本案為此要留下的東西是零**,而且是量出來的:palette 全部是 `:root{--zk-*}` 覆寫(**623 條 / 108 個名稱** / 26 個非空 palette),而本主題已出貨 **862** 個可覆寫 `--zk-*` ⇒ 付費側在主題之後載入自己的 sheet 即成立,不需要本專案提供 hook。**同一輪還裁示移除 `apply(Component, Palette)`**(區域級 palette),見獨立計畫的 Change Log |
| C20 | 2026-08-14 | **P4 的延後項會在 P7 補做** —— A1(tablet 60 條前綴)、A2(1 條孤兒 `-moz-appearance`)「隨 P7 一起處理並計入 P7 的 delta」;B1–B3「明確不做」但**沒有階段歸屬**;C1(`combo` 586 → 191,L-8 的 B 案)、C2(14 處重複/矛盾宣告)排在 P4 之後但**未排程** | **第一期的 CSS 變更範圍封閉:`P4a + P4b + D1/D2 + D4`,除此之外不再有任何一條宣告被增刪。** A1 / A2 / B1–B3 / C1 / C2 **全部改為「只記錄,不做」**,寫進 P8 的 migration guide;A3(S52 的特異性打平)以「**既成狀態**」而非「延後」的方式關掉。連帶 **§P7 的閘門從單一 G-delta 拆成兩段**:tablet 轉換 = **G-zero**、D4 = **G-delta**;`_combo.less` 走 **L-8 的 A 案**逐字轉 | **user 裁示(2026-08-14)**,提案原文與三條裁示逐字收在**計畫書 L2.1〈第一期的 CSS 變更範圍〉**。理由是**訊號純度**:第一期要證明的命題是「CSS 能正確從 LESS 轉換」,任何非必要的增刪都只會稀釋那個訊號。**同一輪強制帶出 keep-vs-revert**(P4a / P4b 已完工進版控,「先都不做」對它們而言是「要不要回退」)—— 裁示 **選項 A:已經移除了就維持**。保留的理由是 P4a / P4b **不在轉換正確性那個命題上**(獨立記帳、逐條可推導、有專屬閘門),回退換來的是敘述漂亮不是證據更強,代價卻是動 48 檔的高風險 revert + 把 7 條**經證明行為中性**的改名放回去。**「承重 vs 選配」這條線必須同時寫下來**,否則「變動儘量減少」會被外推到 D1/D2/D4,第一期就交付不出 compact —— 而那是 L-4 已拍板的對外承諾。**排程副作用:一個問題自己消失了** —— C1 延後之後,原本要決定的「C1 要不要與 P7 同批」不再存在 |
| C21 | 2026-08-14 | **P8 的核帳公式 = 「P4 + P5 + P7 三階段已核准 delta 總和」**,寫在 8 處 `doc/` 敘述 + `tasks/utility-css-vs-p7-ordering.md` | **`P4a + P4b + D1/D2 + D4` 四段。** 同日補上 L3-A 缺的 D1/D2 那一列(**#59**,`1` 檔 / `0 移除 / 352 新增`) | **原式同時犯了兩個方向的錯:漏掉一個貢獻 352 的,又點名了一個貢獻 0 的。** ① **D1/D2 沒被算進去** —— 它 2026-08-12 完工、S56 明寫它是「**第三個已核准 delta**」,但公式從沒更新;② **P5 貢獻 0** —— 它不採 `@scope`,收在 **G-zero**(紀錄 #47/#48),公式卻把它列為三段之一。**更嚴重的是核帳資料源本身缺了那一列**:計畫書 §P8 指名 L3-A 為來源,而 L3-A 停在 #58,**D1/D2 一列都沒有**,其驗收證據在 `tasks/l4-density-mechanism.md` L3.5 而不在 L3-A ⇒ 照原樣走到 P8,**核帳會恰好差 352 條新增**,而那個時點最便宜的錯誤解釋是「P7 做壞了」—— 正是本案一路在防的誤判。**數字是重新量的不是抄的**:`npm run check:gate` **exit 0**;`cssdiff --max-diff 5000` = **48 檔 / 1104 筆 = 745 移除 + 359 新增**;按檔拆 `+` 得 norm 355 / slider 2 / pdfviewer 2,扣掉 P4b 的 7 條改名,norm 剩 **352 = 350**(帶 `data-density`,實測 grep = 350)**+ 2**(DSP `<c:if>` 開關)。**A1/A2 那 61 條「兩邊都不算」現在是正確的** —— 它們一條都沒被動過,不進任何一段 delta。**歷史列不改**:S48(:1047)與 `tasks/l2-browser-support-analysis.md:168` 的舊公式是當時的敘述,依附加式紀律留原文 |
| C22 | 2026-08-17 | **§P7 第一段的驗收 = 「`tablet.css.dsp` 應逐 byte 不變」**(C20 拆兩段時寫下的),同一句話同時說「與 P3 已做過 74 次的驗收完全同形」 | **改成 P3 真正的形狀:`cssdiff` 逐條 0 + 位元組差異全部落在 `classify()` 那份封閉的序列化類別清單內(`UNEXPLAINED: 0`)。** 敘述層的對應紀錄是 **S58** | **那一句自相矛盾,而且採信的是做不到的那一半。** 「逐 byte 不變」隱含「CleanCSS 與 LESS 的序列化器逐字元一致」,而兩者不一致是**早就量過並封閉列舉**的事實(`check-build-css.js` 的 `SERIALIZATION_CLASSES`,五類)。實測全樹只有 **23 / 85** 檔逐 byte 相同,**P5 自己轉的 `norm.css.dsp` 也不在其中** ⇒ 這條標準若當真,連已經收工的 P5 都要被判失敗。tablet 落在其中 3 類(組合子 `>` / `+` 兩側空白、`opacity:.3` vs `0.3`、`rgba(255, 255, 255, 0)` 有無空白),27638 → 27729 B,**零語意差異**;宣告層 `cssdiff` 為 **0**,對原始 `baseline/` 的儀器讀數維持 **48 檔 / 1104 筆**(⇒ 本段沒有貢獻任何一筆差異)。**危害不在「多紅一次」,在於下一步會去修一個沒有壞的東西** —— 唯一能讓 tablet 逐 byte 相同的做法是讓 `build-css.js` 去模仿 LESS 的序列化器,那正是 §2.6 選擇「封閉類別清單」而不是「位元組相等」的原因。**C20 的三條裁示一條都不動**:A1/A2 仍是只記錄不移除,第一段仍是 G-zero,D4 仍是唯一的 G-delta |
| C23 | 2026-08-17 | **§D4 的機制 = 「compact 那套加 `[data-density="compact"]` 前綴,並對只在 default 側的 67 條規則**逐條中和**(明確寫回非平板的值)」**;驗收 = 那 67 條做 mobile UA computed-style 比對 | **改成「兩張完整的表,各包一個 `<c:if>`,由 library-property 二選一」,中和一條都不做。** 驗收改為**三態實測全量比對**:`unset`/`foo` 對 `baseline/` 895 = 895、`compact` 對 `iceblue_c 11.0.0` 869 = 869,三者皆 0 缺 0 多 0 異。代價寫在明處:執行期 `data-density` 屬性不驅動平板層,只有 library-property 驅動 | **三個理由,一個比一個硬。** ① **那份清單的單位是錯的** —— 決定會不會漏的是 **(單一選擇器, 屬性)** 對,不是選擇器字串;`.z-a,.z-b{x}` 對 `.z-a{x}` 看起來是「共用選擇器」卻照樣漏掉 `.z-b`。實測真正的漏面是 **187 條宣告 / 113 個選擇器**,是被點名那份的近 3 倍,而**沒被點名的那些沒有任何東西看得見** —— 驗收清單本身就是那份不完整的名單。② **CSS 沒有「不存在」運算子** —— 那 187 條在 compact 下要的是**不存在**而不是被蓋掉,這正是 P5 處理 `browserDefault` 時已經寫進 `build-css.js` 檔頭的結論:**只有伺服器端條件能刪掉一條規則**。「寫回非平板的值」是對「不存在」的近似,要從其他 84 張表的層疊裡推導 187 個值,推完之後**沒有任何自動化盯著**,上游一改就靜默失準 —— 本案已經有一項(A3)只靠文字擋著,不該再加一項大 3 倍的。③ **保住執行期屬性的兩種寫法都要對 default 路徑動刀**,而那是 100% 現有使用者所在的路徑:`:where()` 前綴等於把下限拉到 Safari 14,與 **L-2 拍板的選項 C(不收窄 ZK 本體的支援範圍)**相牴觸,而且選擇器不被認得時是**整條規則被丟棄**;改用 `html:not(...)` 沒有語法問題,但 246 條各加 (0,1,1) 特異性,可能讓現在輸給桌面表的平板規則翻盤(`.z-column-content` 一族即是疑似案例)。**放棄的那一格今天本來就是壞的**:D1–D3 收工後 `apply(COMPACT)` 已經是「桌面 compact + 平板 default」,本案只修好 property 那條路,沒弄壞任何一條;日後要補是**純附加**。**C20 的三條裁示一條不動**(A1/A2 只記錄、第一段 G-zero、D4 是唯一 G-delta);§D4 的 ~53 KB 成長估算隨機制作廢 —— 兩段互斥,任一模式下服務出去的量與今天同級。對應的敘述紀錄是 **S60**(單位錯誤)與 **S61**(口徑更正),閘門紀錄 **#61** |
| C24 | 2026-08-17 | **平板層的執行期切換是「代價」與「日後純附加的待辦」** —— D4 收工時把「`IceblueDensity.apply()` 不驅動平板層」寫成付得起的取捨,並註明「日後要補是純附加,不擋任何事」 | **升格為規格:平板觸控層的密度只由 library-property `org.zkoss.zul.theme.density` 決定,Java API 兩支方法都不驅動它,而且不列入待辦。** 選項 B / C / D 的評估留作紀錄,不是路線圖 | **user 裁示(2026-08-17)。決定性理由是前一版本來就只有這個規格** —— `@themeProfile` 時代切 compact 的作法是「改 LESS 變數 → 重編 jar → 換 jar」,**平板層從來沒有過任何執行期切換**。所以本規格對平板層是**同等能力、便宜非常多的機制**(一個 library-property 取代第二個出貨 jar),不是能力損失;唯一新增的不對稱是桌面層多拿了執行期切換(D1–D3),不是平板層少了一格,文件要照這個方向寫。**連帶效果是一整批工作消失**:那 187 條寫回(含 60 條沒有來源可抄、只能實測寫死的)只在「要讓執行期屬性也驅動平板層」時才存在,現在全部不必做。**評估數字保留備查**(2026-08-17 實測,對未經本案改動的 `baseline/` 與出貨中的 `iceblue_c 11.0.0`):B(兩側加前綴)= default 路徑 **16** 條回歸;C(只有 compact 加前綴)= compact 側 **13** 條翻盤;D(變數 + fallback,不動選擇器)= compact 側 **9** 條翻盤、187 條裡有 **99** 條可寫成 `var(--zk-*)` 不漂移。三者共通的天花板是 **CSS 變數解析失敗是 IACVT,屬性計算成 `unset`(繼承/初始值),不等於「規則不存在」**;唯一乾淨的解 `@layer` + `revert-layer` 要求整個主題改用 cascade layer,而 `@layer` 已由 §L2.1 列為範圍外。**本裁示不動任何 CSS 或 Java 邏輯 —— 現行實作已經就是這個規格**,閘門數字不變也不需要重跑;文件更動:`tasks/d4-tablet-density.md` L2.3 改寫為規格條、`IceblueDensity` Javadoc 改寫、D5 的 migration guide 要正面寫這條規格。完整推導見 `tasks/d4-tablet-runtime-explainer.html` |
| C25 | 2026-08-17 | **桌面層的 density 有三條路**:library-property(D2)、`[data-density="compact"]` 屬性(D1)、以及執行期 Java API `IceblueDensity.apply()`(D3,已完工並有 Playwright 實測) | **收斂成一條:library-property 是切換 density 的唯一方式,桌面與平板皆然。** D3 整個刪除(含 `IceblueDensity.java` 與 `check-density-runtime.js`),D1 的覆寫區塊改為鍵在 `:root` 並整段包進 D4 的 `<c:if>`,`[data-density]` 屬性自來源樹消失;`build-css.js` 的 `.ZKDENSITY ` placeholder 一併移除 | **user 裁示(2026-08-17),理由與 C24 同源:前一版兩層都沒有動態切換。** `@themeProfile` 時代要切 compact 得改 LESS 變數、重編 jar、換 jar,所以本版是**同等能力、一個 property 取代第二個出貨 jar**,不是能力縮水;未來再考慮動態切換。**連 CSS 的屬性掛勾也拿掉、不只是刪 API,是因為留半套會留一條通往 S36 的活路** —— D4 之後平板層只認 property,若桌面層還留著屬性掛勾,任何人設了那個屬性在行動裝置上就會得到「桌面 compact + 平板 default」,那正是 D4 被設立來消滅的分裂主題。**delta 形狀不變**:仍是 1 檔 / 0 移除 / **350** 新增,只是選擇器與條件換了 ⇒ P8 的核帳公式 `P4a + P4b + D1/D2 + D4` **不受影響**。**額外效益**:預設密度的 app 不再收到那 14473 B(gzip 2403 B)恆送卻永遠選不到的宣告。**代價兩項,已寫進文件**:① 預覽程式側邊欄的即時 density 開關移除,審查 compact 要以 `-Dorg.zkoss.zul.theme.density=compact` 重啟(與客戶真實作法一致);② D3 的實作與測試刪除,git 留著。**驗證**:`check:gate` / `check:bytes` / `check:build-css` / `check:density-css` / `check:doc-refs` 全 0;`check:density-property` 三態改判為「區塊在不在」——未設**沒有**、`compact` **有一個且鍵在 `:root`**、`foo` 同未設,並跑了負向控制(把區塊改掛到 default 條件 → exit 1,三態全部被點名);`check:tablet-density` 不受影響。`readme.md` 的〈switch to compact profile〉整章改寫(順手兌現 D5 的一部分),`norm.css` 檔頭改為「不要在這裡切」。執行計畫 `tasks/d6-property-only-density.md` |
| C26 | 2026-08-17 | **§P8「`build-css.js` 長出 `.less` 分支接手引擎的工作」**,並為它記了三個地雷(不可前置 taglib header、不可套 `HOSTILE_CONSTRUCTS`、`~./` 改寫必須維持 entry-buffer-only) | **不寫這個分支。** 三個地雷連帶作廢(它們全是在描述一段不存在的程式碼)。引擎的另外兩塊貢獻(`~./` shim、建置流程)也不必接:沒有輸入就沒有要走的路徑 | **P7 讓 entry `.less` 歸零之後,這個分支的輸入集合是空的** —— 寫了它就是一段**永遠不會被執行**的程式碼,而那正是本計畫 L2.0〈儀器證明〉整節在防的東西(P2 已經實測過:沒有輸入時閘門的 PASS 是免費的,把 `minify()` 改成 `return ''` 都照樣 exit 0)。**這條斷言是在還有 3 個 holdout 的時候寫的,那時它是對的**;是 P5 / P6 / P7 把 holdout 一個一個清掉,才讓前提消失。**實測依據**:P8 前 `zklessc` 已經 `compiled 0 file(s)`、`check:build-css` 讀數 `85 converted (real source), 0 reconstructed from LESS`、`passthrough: 0`。**連帶清掉的既有死碼**:`check-build-css.js` 開頭那兩步 LESS 重建(編譯全樹 + 剝 taglib header)同樣沒有輸入,一併刪除,`hasLessEntry()` / `DSP_DIRECTIVE` / `lessOut` 三個孤兒隨之移除 —— 並補一條**非空轉**的新斷言取代它們(輸出檔數必須等於 entry 來源檔數,不等就 exit 1)。**pin 的理由也要跟著改**:`less` 4.8.1 仍是直接 devDependency,但不是因為「`.less` 分支要呼叫 `less.render`」(那是 workflow 腳本 P1 那一列的原文),而是因為 **`scripts/less2css.js` 這支要留給 fork 重跑的轉換工具直接 `require('less')`** |
...
原稿的警告原文保留(**注意**:裡面的 `L19、L280、L712` 是**舊行號**,2026-08-04 的三層式
重構之後已失效;`§4.4 / §6 / §8.1` 指的是那份不存在的評估文,不是本文件的章節):
...
<details>
<summary>為什麼記在被追蹤的 <code>doc/</code> 而不是 <code>tasks/</code>、規範與狀態為什麼分離、進度文件的形狀</summary>
...
- 每跑一次 `cssdiff` 就在**附加式**的〈閘門紀錄〉段加一行(階段、baseline、差異檔數、差異條數)。
  P8 的最終核帳要靠這份紀錄把 ~~P4 + P5 + P7~~ **`P4a + P4b + D1/D2 + D4`**(**C21** 更正)
  的 delta 加總對上 —— 沒有逐次紀錄就核不了帳。**這條規則被違反過一次**:D1/D2 完工當時
  漏記,2026-08-14 以新列 **#59** 補記(日期寫補記日,不回填),所以「每跑一次就加一行」
  **不是形式,是核帳的必要條件**。
...
#### 2026-08-04:L3 拆成獨立附錄檔

三層式重構之後,L3 在單檔裡佔到 **83%(計畫書)/ 84%(進度書)**,而計畫書的 **2063 行**
已經超過 Read 工具 2000 行的預設上限 —— **用預設參數讀就會靜默截斷尾巴**,不報錯。
workflow 又要求每個子代理「READ THESE FIRST」兩份主檔,等於光讀文件就要 ~78k token。

所以 L3 移到 [iceblue-drop-less-plan-appendix.md](iceblue-drop-less-plan-appendix.md) 與
[iceblue-drop-less-progress-appendix.md](iceblue-drop-less-progress-appendix.md),
主檔只留 L1 + L2 + L3 索引(415 / 226 行),讀取成本降到 ~13k。

**分層沒有變** —— `plan-spec` 約束的是分層(細節不准散進 L1/L2),不是檔案數;
附錄仍然是 L3 這一層,而且明文宣告不定義任何規範。索引的〈何時要看〉欄
從此兼任「要不要打開附錄」的判斷依據。量測、被否決的更細拆法、以及付出的代價見
