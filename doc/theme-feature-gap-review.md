...
| Doc path drift in `doc/css-dsp-file-structure.md` | hygiene | Lists legacy iceblue_c paths for `tbeditor`/`cropper`/`signature` (`inp/`, `med/`, `wgt/`); ZK 10.2.1 resolves them via widget packages (`zkmax/tbeditor/`, `zkmax/cropper/`, `zkmax/signature/`). Marble builds the **correct** paths; the **doc** is stale. → **P3**. |
...
| ID | Item | Why | Rough effort |
|----|------|-----|--------------|
| **P3-1** | Migration-decision note in `DESIGN.md §7` | Record that deprecated `org.zkoss.zul.theme.*` font props are intentionally absent (CSS-token design) so no one re-adds them. | XS |
| **P3-2** | Refresh `doc/css-dsp-file-structure.md` paths | Replace legacy iceblue_c paths (`tbeditor`/`cropper`/`signature`) with ZK 10.x widget-package paths; drop the misleading `splitter.css.dsp` entry. | S |
| **P3-3** | Delete stray `src/main/resources/web/js/zul/layout/anchorlayout.css` | Removes a dead DSP emitted at an unrequested path. | XS |
| **P3-4** | Add MD3 `--zk-typescale-*-letter-spacing` tokens | Tokenize tracking; replace hardcoded `.z-h1`/`.z-h2` literals. | S |
| **P3-5** | (cosmetic) Explicit `ThemeOrigin.JAR` in `register`; `tbeditor.css` mono-family token | Self-documentation / token consistency. | XS |
