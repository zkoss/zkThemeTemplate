...
| M6 | `.z-timepicker-input` bbox does not extend past the root's rounded corner on the left edge (root `overflow: hidden`) | the always-readonly opaque input must be clipped to the root border-radius — no square corner bleed |
...
| c5 | `.z-timepicker:focus-within::after` | box-shadow | `inset 0 0 0 1px var(--zk-color-primary)` | reference/focus-affordance-no-layout-shift.md (opaque-child caveat) — the focus ring is drawn on an always-present `::after` OVERLAY (transparent at rest), not a root inset shadow: the always-readonly input has an opaque bg that would occlude a root inset shadow on the input side (ring looks thicker around the transparent button). The overlay paints above both children (`pointer-events:none`, clipped to the rounded shape by `overflow:hidden`). Border stays 1px → no layout shift. |
...
| c5d | `.z-timepicker:focus-within` | box-shadow | `none` | reference/focus-affordance-no-layout-shift.md — the ring must NOT be a root inset shadow (the opaque always-readonly input occludes it on the input side, making the ring asymmetric); it lives on `::after` (c5) instead |
...
| c15 | `.z-timepicker-readonly .z-timepicker-input` | background-color | `var(--zk-color-surface-container)` | DESIGN.md §3 — readonly inputs use surface-container tint |
| c18 | `.z-timepicker` | overflow | `hidden` | components/timepicker.md — Timepicker is always-readonly (constructor forces it), so the input ALWAYS carries the opaque surface-container bg (c15); without clipping, the input's square corners paint over the root's `border-radius` (4px) on the left edge and the rounded corner reads as missing. `overflow:hidden` clips the child to the radius. |
...
| readonly | `.z-timepicker.z-timepicker-readonly .z-timepicker-input` | c15 |
