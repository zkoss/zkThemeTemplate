...
**測試對照**（`component-theming.spec.ts`,49 綠,已用 `npx playwright test component-theming.spec.ts` 實測確認):button(regional/disabled/whole-app 共 3)、
...
toast(regional/whole-app 共 2)、a(regional/whole-app 共 2)。
**chip 無專屬斷言**(見上,region 不可展示;可日後補 inline 斷言)。**messagebox、popup 同樣無 region 斷言**(結構性原因見對應列說明,非疏漏)。
