...
| 4.4b | `.z-label` re-declares what it already inherits from `body` (`zul/src/main/resources/web/js/zul/wgt/css/label.css`; full write-up `zk/doc/marble-theme-followups.md` item 6 — renamed from `harness-followups.md` in `zk` `67dd066352`, "harness" already meaning `doc/harness/` in that repo; deferred to P4 by the user, zk-05's chat D6, 2026-09-11). Stage 1: delete only the redundant `color: var(--zk-color-on-surface)` — token-different from `body`'s `--zk-color-on-background` but the same computed value, so the rule is a no-op today and its sole effect is blocking every contextual override; this is the root cause of 52 dead text colours on `z-bg-*` container wrappers, and deleting it fixes all 52 with no new utility classes (the paired `z-text-on-*-container` utilities option was rejected for forcing a class-pairing MD3's container/on-container model exists to avoid). Stage 2 (only if stage 1 is clean): evaluate also deleting the four font properties, which would make container typography work generally and mean commit `47d26068ed`'s 1223-page typography sweep was symptom-level, not required — if stage 2 lands, update `.claude/skills/marble-theme/reference/zul-authoring.md`'s "ZK wraps raw text in a Label" section, since its premise changes. **Ordering: must land before 4.5** — this is shipped theme CSS, not a preview-only change, so if the D18 sync runs first, the template inherits the stale rule | 1 rule (stage 1); ≤ 5 properties (stage 2) | Sonnet | **Opus** | Per `zk/doc/marble-theme-followups.md` item 6's own "Verification bar": `probe.js` on a `z-bg-*` wrapper shows the inner `.z-label` computing the container's paired `on-*` colour, not `rgba(0, 0, 0, 0.87)`; every Playwright project that passed for `47d26068ed` stays green (`smoke` 115, `framework` 7, `responsive` 4, `print` 2, `zindex` 2, `gallery` 82 — only the pages whose text colour legitimately changes get their baseline regenerated, with bare `--update-snapshots` on Playwright 1.59.x, never `=all`, per F75/lessons.md 2026-09-11); `cd zktest && ./gradlew test` green (this is shipped CSS, so the component suites are in scope, unlike a preview-only item); both themes and `forced-colors` checked (`_forced-colors.css`'s own colour override must still win) |
...
the oracle is final (D61 A′ still promises a scoped re-cut). `zkpreview`'s harness copy points at
`zk/zkpreview/doc/screenshots` (`playwright.config.ts:13`, `../../../doc/screenshots` from the spec directory — not `zk/doc/screenshots`, as first written), so until 3.18b `toHaveScreenshot` in `zk` can only create
...
F20 measured 23 of 97 files citing template paths. Same families, second map file, `apply-path-map.js`, gated like 3.5. **Addendum 2026-09-11:** measured on a scratch copy before dispatch, the 3.2 map + fixes table already rewrite everything the skill cites (42 substitutions in 31 files; F20's 23 counted only the `src/` forms) — the second map file is not created; five fixes rows cover the remainder (see row 3.19).
...
### D204 — Three template scripts the agents and `doc/spec` cite are MAP identity rows that P1 never copied — **RULED 2026-09-11 (chat D204-A): port them (row 3.21)**

`scripts/check-icon-coverage.sh`, `scripts/js-source-hash.sh` and `scripts/render-iceblue-baseline.sh` map to themselves in
the 3.2 map, but P1 ported only `build-css.js` and `check-css-dsp.js`; `zk-theme-evaluator` (Gate 0b recomputes the
`js-source-hash`), `zk-spec-author` (2 citations), `icon-index.md` and `icon-policy.md` (3) would call tools that do not
exist in `zk` after 3.10–3.14 and 3.6. **A (ruled)** — copy the three (and `capture-iceblue.js`, which the third one
calls) into `zk/scripts/` as row 3.21, rewritten for the `zk` layout; `js-source-hash.sh` changes meaning on purpose: the
template hashed the pinned jars because its checkout ran ahead of the release, whereas in `zk` the checkout is the
widget being styled. **B** — turn the citations into "template-only tool" notes: Gate 0b would be dead in `zk` and the
P3 drill could hit it.

### D205 — Who lands the D69-B forced-colors fix in `zk` — **RULED 2026-09-11 (chat D205-A): the P3 session, row 3.20**

The template session's D69 was ruled B by the user (the fix lands in `zk`, the template stays as it is; hand-over §G).
The change is three selectors in the `(2a focus)` rule of `zul`'s `tokens/_forced-colors.css` — outside the P3 copies'
footprint, so it needed its own ruling. **A (ruled)** — row 3.20 here: Sonnet edits the CSS, the Planner proves RED on
the pre-fix tree with a computed-style probe (deterministic, unlike F61's pixel sample), Opus judges GREEN plus three
