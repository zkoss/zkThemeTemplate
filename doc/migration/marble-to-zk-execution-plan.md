...
| 3.5 | Apply the path-rewrite map to the copied skill; add the `zkpreview` section to `reference/verification.md` in the copy (D48: port 8085, `./gradlew appRun`, `PREVIEW_URL=http://127.0.0.1:8085`, live-reload absent; **and one measurement rule, F57: geometry claims are read off the captured PNG, never the live DOM — `getBoundingClientRect` at `networkidle` runs before the gallery spec's font-settle wait and reads 1 px wide on the splitter page**) | map + 82 | Sonnet | **Opus** (2026-09-11 switch) | `grep` for every *source* prefix in the copy = 0; every rewritten path exists in `zk` (the existence loop already used today); the copy's `verification.md` states the `zkpreview` facts and the stated command works | Target |
...
| 3.18 | Copy the twelve `doc/` paths the map lists outside `doc/spec` and `doc/contracts` — 6 root `doc/*.md`, `doc/harness/` (4), `doc/screenshots/` (2) — into `zk/doc/` (F17; ruled 2026-09-10, chat D12-B). Run before 3.6 so `doc/spec/index.md`'s four outward links resolve | paths | Sonnet | Sonnet | each of the 12 paths exists in `zk/doc/` and `cmp` against the original is silent | Target **Caveat (F61, 2026-09-11): `doc/skill-gaps.md:308` ("checked and clean: tree row, organigram node") is known wrong; copy the file after the template's correction (its D69) or re-sync that one file later — never patch the copy.** |
...
session's chat D-numbers start at **D200**; the P2 session's continue from D69, the template session's from D69
upward as well — each states its last-used number in every cross-session message.
