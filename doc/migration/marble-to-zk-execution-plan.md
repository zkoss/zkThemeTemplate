...
| 2.6 ✂ | Zero-tolerance comparison — **gallery family, 99 baselines** — the template's own harness run against `zkpreview` (D49) | output-heavy | Sonnet | **Opus** | a diff ledger file: one line per baseline, `IDENTICAL` or `FIXED <commit>`; **every difference fixed** (chat D57) — a row may read `OPEN` only while the Planner has the image pair in front of the user as a decision |
| 2.7 ✂ | Zero-tolerance — **state family, 70** (hover / focus / active) | output-heavy | Sonnet | **Opus** | same ledger shape and the same fix-until-identical rule (D49) |
| 2.8 ✂ | Zero-tolerance — **tablet family, 30** (mobile UA project) | output-heavy | Sonnet | **Opus** | same; tablet's two known 2 % opt-ins are decisions for the user (image pair shown), not ledger explanations (D49) |
