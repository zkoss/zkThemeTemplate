#!/usr/bin/env python3
"""summarize-cmp.py <compare.json> — one line per PNG from compare.config.ts: IDENTICAL, or the pixel
count / size mismatch / NO-BASELINE that Playwright's comparator reported. Exit 1 if anything differs."""
...
sys.exit(0 if c.get('IDENTICAL', 0) == len(rows) else 1)
