...
static)
  ...
  for f in "$HSRC"/*.ts; do b=$(basename "$f")
    case "$b" in
      ...
      render-smoke.spec.ts)   # D206-A: the header's "how to start the preview app" line differs by build tool (template: Maven; zk: Gradle/gretty via zkpreview/start.sh) — no single command starts both
        extra=$(diff "$f" "$HDST/$b" | /usr/bin/grep -E '^[<>]' | /usr/bin/grep -vE "^< //   withjdk\.sh 17 mvn test exec:java@preview-app\$|^> //   cd zkpreview && \./start\.sh\$" || true) ;;
  ...
  ok "harness: 15 files; byte-identical apart from the 3 WEB_DIR lines, the config's baseURL default, (2.10) the four theme-prefix lines of screenshot.spec.ts, and (D206-A) render-smoke.spec.ts's start-command comment line"
