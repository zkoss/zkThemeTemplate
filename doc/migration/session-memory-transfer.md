...
- **Open items a later Planner inherits:** D69 (template session's forced-colors sweep — if ruled A, the CSS lands in the template first and reaches zk as one re-sync commit; `doc/skill-gaps.md:308` is known wrong until then); D43 (the Jira draft for the ZK-6112 tracker text, in zk `tasks/`, awaiting the user's review); P4 follow-ups recorded in gates 2.9 / 2.11 / P2 (test tightening `reset == wcs - 1`, `CSS_URI_BACKED` naming, skeleton tokens' dark mode, optional third ledger column); the P4 re-sync rule for `doc/screenshots` (count and copy from `git ls-files`, never a live tree — F66).
