...
- Root clips to its border-radius (`overflow: hidden`) to trim the focus-ring overlay (and any child background) to the rounded corners. Historically the readonly input carried an opaque `--zk-color-surface-container` background whose square corners bled past the left radius, which is why clipping was added; the theme now renders readonly as an **active, transparent-input** field (see `doc/contracts/timepicker.md` + `doc/combo-readonly-consistency-audit.md`), so the input is transparent in every state and `overflow: hidden` remains only to clip the `::after` focus ring.
